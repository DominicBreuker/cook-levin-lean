...
/-!
# The SAT verifier as a program

`evalCnfCmd` decides `satisfiesCnf a N` from the encoding `encodeState (N, a)`: it walks
the clause stream and, for every clause, checks whether some literal is satisfied by the
assignment held in unary. `evalCnfCmd_decides` and `evalCnfCmd_cost_bound` are its
...
/-! ### UNARY, bit-level encoding

Every cell is `0`/`1` so the encoded state is a `Compile.BitState`. Numbers (variable indices) are therefore **unary** blocks
...
The frame is 16 registers (`regBound = 16` in `EvalCnfTM.evalCnfDecidesLang`).
`encodeState` lays out the first 12; registers 12–15 are scratch, read as `[]` and pad on
first write.
...
/-! ## The inner-body contracts

The assembly below (`evalCnfCmd_decides`, `evalCnfCmd_cost_bound`,
...
/-- ** `processOneLiteral` behaviour.** With one encoded
...
/-- ** `processOneLiteral` cost** — quadratic: the
...
/-- ** `processOneClause` behaviour.** This
...
/-- ** `processOneClause` cost** — cubic in
...
/-- **** `processOneClause` touches only registers `0–15`. -/
...
These discharge the `enc_bit` obligation
...
/-- **Cost bound.** The loop-cost tool is `Cmd.cost_forBnd_le` (a *uniform*
...
quadratic). Hence the quartic budget with an explicit constant: `|N| · 1000·(7n+1)³ + |N|² + 3 ≤ 200000·(n+1)⁴`. -/

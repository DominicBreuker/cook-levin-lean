...
/-!
# Recognising encoded CNFs

`scanStep` is a small finite automaton with one counter that reads a cell stream left to
right; `wfCnfB_iff` proves that it accepts exactly the bit lists of the form `encodeCnf N`.
`parseTotal` parses an accepted list with `CnfSerialize.decCnf` and returns the
unsatisfiable CNF `botCnf = [[]]` on every other list. The `SATStr` verifier uses the
automaton to recognise well-formed inputs.
-/

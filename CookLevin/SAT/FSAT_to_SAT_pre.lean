...
/-!
# The Tseytin transform in pre-order

`PreTseytin.preTseytin b f` is the Tseytin CNF of the formula `f` whose auxiliary
variables are numbered by the position of each subformula in the pre-order serialization,
offset by `b`. `preTseytin_correct` proves `FSAT f ↔ SAT (preTseytin b f)` for
`b > formula_maxVar f` and `preTseytin_size_le` bounds its size. This ordering is what a
single left-to-right pass over the serialized formula can produce.
-/
...
theorem preTseytin_size_le (b : Nat) (f : formula) (hb : formula_maxVar f < b) :
  ...
  -- carrier is `var` and omega goes blind

...
/-!
# The canonical encoding of CNFs and its parser

`encodeCnf N` writes a CNF as bits: each literal is its sign followed by its variable index
in unary, literals and clauses are closed by delimiters. `decCnf` parses such a bit list
back and `decCnf_encodeCnf` proves it inverts `encodeCnf`.
-/
...
instance instSerializeCnf : Serialize cnf where
  ...
  -- that no polynomial slack is needed.
  sizeLB := fun n => n

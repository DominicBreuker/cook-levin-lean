...
`SATStr x := SAT (cnfOf x)` where `cnfOf x` parses the bits of `x` as a CNF and yields a
fixed unsatisfiable CNF if `x` is not an encoding (`satStr_iff`: `SATStr x ↔ ∃ N, bits of x
= encodeCnf N ∧ SAT N`). The verifier program `satStrMfc ;; evalCnfCmd` recognises a
well-formed input, decodes the certificate as an assignment and evaluates the CNF;
`inNPCmd_SATStr` packages it as an `NPWitnessStr SATStr`.
-/
...
open CookLevin.Lang EvalCnfCmd CnfWellFormed

/-! ## The language -/

/-- The CNF a bit string denotes: the canonical parse when the string is an encoding
(`CnfWellFormed.parseTotal`), and the unsatisfiable formula `[[]]` when it is not. -/
def cnfOf (x : List Bool) : cnf := parseTotal (strBits x)

end SATStr

/-- **SAT as a language of bit strings**: `x` is in `SATStr` when it spells out a satisfiable
CNF formula, in the encoding `EvalCnfCmd.encodeCnf`. -/
def SATStr (x : List Bool) : Prop := SAT (SATStr.cnfOf x)
...
`WCUR = l.drop i`, the two flags, and the tally. -/
...
theorem size_cnfOf_le (x : List Bool) : encodable.size (cnfOf x) ≤ encodable.size x + 1 := by
  ...
  · have h1 := CnfSerialize.size_le_encodeCnf_length (cnfOf x)
    have h2 : encodeCnf (cnfOf x) = strBits x :=
      encodeCnf_parseTotal _ (strBits_bit x) hwf
    have h3 : x.length ≤ encodable.size x := CookLevin.length_le_size x
...
theorem strEIn_size_le (v : List Bool × List Bool) :
  ...
  rcases v with ⟨x, c⟩
  have hx : x.length ≤ encodable.size x := CookLevin.length_le_size x
  have hc : c.length ≤ encodable.size c := CookLevin.length_le_size c
  have hn : encodable.size ((x, c) : List Bool × List Bool)
      = encodable.size x + encodable.size c + 1 := rfl
  rw [strEIn_size, hn]
  omega
...
/-- The re-encoder's cost, as an existential polynomial. -/
...
/-- **The `FreePrecomposeData`.** `eIn` is the canonical string layout; all the work is
in `mfc`. -/
...
/-- **The witness: `SATStr` is presented by a verifier program** over the canonical
one-register input layout. -/
noncomputable def satStrWitness : CookLevin.Lang.NPWitnessStr SATStr where
  rel := satStrRel
  dBound := strBound
  ...
  encX_size := fun x => by
    have h : State.size (certState x) = x.length := CookLevin.Lang.State.size_certState x
    have h2 : x.length ≤ encodable.size x := CookLevin.length_le_size x
    have h3 := le_strBound (encodable.size x)
    omega
  sizeLB := fun n => 2 * n
  sizeLB_poly := inOPoly_mul (inOPoly_const 2) inOPoly_id
  encX_sizeLB := fun x => by
    rw [CookLevin.Lang.State.size_certState]
    exact CookLevin.size_le_two_mul_length x
  encX_canonical := fun _ => rfl

/-- `SATStr` is in the class `inNPCmd`, the hypothesis class of `NPhard`. -/
theorem inNPCmd_SATStr : CookLevin.Lang.inNPCmd SATStr := ⟨satStrWitness⟩

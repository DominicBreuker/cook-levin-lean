...
/-!
# Properties of the machine model

Theorems about `FlatTM` and `stepFlatTM` that pin down the intended reading of the model:
the left component of a tape is always empty, the tape is one-way infinite with a wall at
cell `0` and a blank that is not the symbol `0`, a step changes at most the cell under the
head and moves the head by at most one, symbols outside the alphabet make the machine
stick, the state stays below `states`, a run is determined locally by the initial
configuration, and halting is absorbing.
-/

set_option autoImplicit false

namespace CookLevin.MachineFaithfulness
...
end CookLevin.MachineFaithfulness

...
| `#print axioms CookLevinHonest.CookLevin''_of_decodesAssgn` | `[propext, Classical.choice, Quot.sound]` — **the WHOLE honest theorem, reduced to ONE register equation** (2026-07-30). `NPcomplete'' SAT` follows from `EvalCnfSplit.DecodesAssgn certDecode`, a `_run` lemma about an 11-op program. Hardness (`SAT_NPhard''`) is unconditional; the membership half is assembled in `Deciders/EvalCnfSplit.lean` (sorry-free). |
...
| **S4** | **membership half of the honest headline** — `inNPLangFreeSplit SAT`. Without it `NPcomplete'' SAT` cannot be stated and the honest hardness line has no headline to feed. | `Deciders/EvalCnfSplit.lean`, `CookLevinHonest.lean` | **Down to ONE register equation (2026-07-30).** The split layout, `xWidth = 3`, `polyCertRel SAT satRel`, the decoder's cost (`Cmd.chk`, `by decide`) and frame, the frame half of the bridge and all four composite `DecidesLang` bounds are proven and axiom-clean. Open: `EvalCnfSplit.DecodesAssgn certDecode`. Honesty verdicts to record in the audit (plan step 3): `satEncX`'s register 1 is the *derived* `1^\|N\|`, honest only because the live `evalCnfDecidesLang` already requires `CLAUSE_TALLY` in its own `encodeIn`; and `InNPWitnessLangFreeSplit.encX_size` is bounded by `dBound` (degree ≈ 8, measured slack `23` vs `≥ 200000`), so that field constrains nothing and is **not** an honesty check. |

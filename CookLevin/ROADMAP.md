...
| **C7** | **verifier bodies** — `evalCnfCmd` (SAT, gates `inNP SAT`), `cliqueRelCmd`. | **EvalCnf: assembly PROVEN (2026-06-09)** — all `evalCnfDecidesLang` fields discharged from the pinned `processOneClause_*` contracts (EvalCnfCmd.lean); remaining = the 3 inner-body `Cmd`s + pins (bottom-up; budget quartic `200000·(n+1)^4`, `regBound 16` — see HANDOFF). `cliqueRelCmd` still raw stubs (apply the same template — HANDOFF top-down task 1). Gated on C2 for the TM-level bridge. |

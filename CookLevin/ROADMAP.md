...
- **At a chain END, `decodeOut`/`encodeIn` should come from a `Serialize`
  ...
  bit-level, and the two-sided size sandwich `size x ≤ |enc x| ≤ encLen (size
  x)`); `Deciders/CnfSerialize.lean` is the live instance and the worked example
  of writing a parser + `dec_enc`. Risk S5's *tail* half is therefore no longer

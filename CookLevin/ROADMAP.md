...
1. **Finish the compiler (C2). — ✅ EFFECTIVELY DONE (2026-07-02).** The
   residue-contract compiler chain (`Compile_run_physical_residue` →
   `bitDecider_run` → `paddedBitDecider_run`/`paddedCompute_run`) is proven for
   all 9 live ops; the value-as-length trio (`takeAt`/`dropAt`/`consLen`) is
   **retired** (never needed by any remaining witness — 2026-07-02 audit) and
   isolated behind the proven `Op.IsSupported` wall. Remaining C2 work is
   *cosmetic*: delete the trio ops + both walls (HANDOFF, next bottom-up).

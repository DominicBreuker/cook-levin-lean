...
| Genuine `sorry`s (Group C) | **9 in built code**: 2 on the live path (`red_inNP`'s `inTimePoly` half, `hasDeciderClassical`), 3 in dead code (`MultiToSingle`), 3 S1 stage markers (`S1Program.lean`: `stageC` plus its `_run` / `_usesBelow` contract — only the `def` is a real gap), and 1 cost obligation (`S1Witness.s1_reductionLang.cost_le`). **`Simulators/CookTableau.lean`/`GuessTableau.lean` are fully `sorry`-free** — `cookTableau_correct`, `guessTableau_correct`, and **both size bounds** (`≤ (2·(n+1))^10`, 2026-07-24) all sorry-free & axiom-clean |

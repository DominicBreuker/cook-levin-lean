...
| **C7** | **verifier bodies** — `evalCnfCmd` (SAT, gates `inNP SAT`), `cliqueRelCmd`. | **EvalCnf: ✅ DONE (2026-06-10)** — `EvalCnfCmd.lean` sorry-free; `evalCnfDecidesLang` axiom-clean (budget quartic `200000·(n+1)^4`, `regBound 16`). **CliqueRel: ENCODING ✅ DONE (2026-06-29)** — `cliqueRelEncode` concrete + bit-level + probe-validated (`probes/CliqueRelProbe.lean`); the encoding-side `DecidesLang` fields (`encodeIn_size`/`enc_bit`/`width_le`/`regBound`, quartic `timeBound`) PROVEN & axiom-clean. **Remaining: the program** `cliqueRelCmd` (the probe-validated 5-check verifier, still `sorry`) + its behaviour/cost/frame fields — apply the proven EvalCnf template (HANDOFF top-down Task 1). TM-level bridge gated on C2 only. |

...
| | |
|---|---|
| `lake build` | ✅ green (~10 min from cold; `S1Witness.lean` alone spends ~3 min in the kernel on the cost ladder's `decide`) — **and since 2026-08-02 it is the soundness gate**: `#assert_library_axiom_clean Complexity` at the bottom of `Complexity.lean` fails elaboration unless every declaration in every module is `sorry`-free and uses only `propext`/`Classical.choice`/`Quot.sound` (**12 680** declarations, **106** modules, ~2 s). `Complexity/SoundnessGate.lean` gates the endpoints individually, `Complexity/HonestyGate.lean` pins risk S5's two audited functions (`encodeIn`/`decodeOut` of the composite, incl. `encodeIn x = certState x` under `NPhardStr`) as gated `rfl` theorems, `Complexity/CostFaithfulness.lean` gates `Compile.cost_is_time_proxy`, `Complexity/NonVacuity.lean` gates non-vacuity of the `NPhardStr` hypothesis, `Complexity/StatementGate.lean` (2026-08-06) gates the reviewer's *reading list*, `Complexity/StatementMeaning.lean` (2026-08-07) gates the checkable verdicts of the **audit** of that list, and `Complexity/GateSurfaceGate.lean` (2026-08-08) gates what each of those instruments costs a reviewer to read *beyond* the headline, and `Complexity/MachineFaithfulness.lean` (2026-08-09) gates the defining properties of the machine model itself — locality, finite alphabet, finite control, space ≤ time. **Nine** obligations, no CI step and no `grep` involved — see `Complexity/Meta/AxiomGate.lean` and `Complexity/Meta/StatementSurface.lean`. |
| **is `FlatTM` a Turing machine? (S10)** | ⚠→✅ **INSTRUMENTED 2026-08-09 (top-down), and the go/no-go on the simulation is written down.** After FINDING AZ this was the **last** irreducible model question on a reviewer's trust list, and the only answer was "read `MachineSemantics.lean` carefully" plus five point witnesses in `StatementMeaning.lean` §3. `Complexity/MachineFaithfulness.lean` replaces that with **ten universally quantified, exactly metered theorems** covering each property a textbook uses to *define* a deterministic single-tape Turing machine: the head reads one cell and it is the cell it is on; a step changes **at most the head cell** (`tapeCell_tapeStep_of_ne`); the head moves by **at most one** and can never leave cell `0` to the left; the tape grows by **at most one cell**; the transition entry is selected by `(state, symbols read)` and nothing else (`find?_congr_of_read`); the alphabet is finite, closed under a step, and **enforced** — a symbol `≥ M.sig` stalls a valid machine (`stuck_of_symbol_ge_sig`); the state set is finite and closed (`runFlatTM_state_lt`); and therefore **space ≤ time and the head cannot jump** (`runFlatTM_init_local`). ★ **The asymmetry that makes this the right instrument:** a model *weaker* than a TM makes the hardness conjunct **stronger** (the reduction is computed by a more restricted device), so the only dangerous direction is *stronger* — and that is exactly the direction locality closes. ★ **Metered at 2 definitions beyond the headline** (`GateSurfaceGate.lean` §2, ten exact deltas): nine of the ten gates add `tapeCell` or nothing, the tenth adds `tapeSymbolsBounded`. It is a **reading** gate in the sharpest sense FINDING BA allows — an answer to a model question stated in the model's own vocabulary. Evidence and go/no-go: `probes/MachineFaithfulnessProbe.lean`. |
| **FINDING BB (2026-08-09)** | ★ **`FlatTM` is the class of deterministic single-tape Turing machines RESTRICTED TO APPEND-ONLY TAPES — not the full class — and this had never been said.** A textbook TM's step effect is a function of `(state, symbol read)`: write `s`, move `d`. Ours is not. `probes/MachineFaithfulnessProbe.lean` §2 is the whole finding in four lines: on `([], 1, [5])` and `([], 2, [5])` the head reads the **same** symbol (blank, `none`), and the **same** write `some 7` appends on the first and is **dropped** on the second. The step therefore consults one bit the finite control does not have — where the frontier is. Two consequences. (a) **It is safe**, and provably in the right direction: the machine *loses* a write it would otherwise have made, so "computable by a `FlatTM` in polynomial time" is a **stronger** claim than the textbook one and the hardness conjunct holds a fortiori. Nothing on the proof path needs the converse — the hypothesis class `inNPStr` is stated over `Cmd`, and the tableau construction consumes an arbitrary `FlatTM` rather than producing one. (b) **It is the obstruction that decides the simulation question**: a `Turing.TM0` machine's action *is* a function of what it reads, so no simulation over the plain alphabet `Option Nat` can reproduce it; the frontier has to be marked in the alphabet. ⚠ This is also the reason `MachineFaithfulness.find?_congr_of_read` must be read as being about the **entry selected**, never about the tape effect — the two come apart exactly here. **The generalisable lesson: when a model's step is "obviously" read-determined, check it on the two configurations that read the same symbol for different reasons. The restriction that makes a construction sound (here: locality, which the 2026-07-17 fix bought) is a restriction on the MODEL, and it belongs in the model's own documentation rather than only in the docstring of the function that implements it.** |
| **the go/no-go on a Mathlib `TM0` simulation (S10)** | **VERDICT 2026-08-09: FEASIBLE, ~2–4 sessions, and it buys a SECOND OPINION rather than a new guarantee — so it is not the next thing to build.** The question the handoff posed was whether to exhibit a simulation between `FlatTM` (at `tapes = 1`, all `Compile` emits) and Mathlib's independently specified `Turing.TM0`, in the direction *theirs simulates ours*. Answers, each pinned against Mathlib's real definitions in `probes/MachineFaithfulnessProbe.lean` §§4–6. **Q1 tape shape:** configurations map injectively (`Γ := Option Nat`, `default := none`; our `right` holds no blanks, so `ListBlank`'s quotient collapses nothing and the frontier is recoverable as the first blank) — but **steps do not**, by FINDING BB. **Q2 symbols:** no obstruction; `Γ := Option (Fin M.sig)` is available for any valid machine and `tapeStep_bounded` is exactly the closure property needed. ⚠ One thing the probe turned up: `validFlatTM` bounds the symbols in the transition *table*, not on the initial *tape*, and `ComputesBy.computes` calls `runFlatTM` directly, bypassing `execFlatTM`'s `isValidFlatTapes` check — so a witness's `encode` *can* put an out-of-alphabet symbol on the tape. Harmless, and `stuck_of_symbol_ge_sig` is why. **Q3 direction and cost:** four obstructions — one action per TM0 step (2 steps + 1 label each, a constant factor, so polynomial bounds transfer), no `Nmove` (free: write back what was read), the left wall (one marker symbol or shift the input), and append-only (one frontier marker + a two-step dance). All bookkeeping; none an obstruction in principle. ★ **Why it still comes second:** what a reviewer needs is that our step is not *stronger* than a TM step, and the locality theorems of `MachineFaithfulness.lean` give that directly and in the model's own vocabulary. Obstruction 4 shows the simulation would largely be proving our model is *weaker* — the safe direction nobody was worried about. **The generalisable lesson: before porting a model into another library's model, ask which direction of the correspondence the theorem actually consumes. Half of a simulation may be proving something you already have, and the structural properties that define the model can be cheaper to state AND cheaper to read than the correspondence.** |
| **the gates themselves are metered (S9)** | ✅ **NEW 2026-08-08 (top-down).** `Complexity/GateSurfaceGate.lean`. Every instrument a reviewer is told to believe is a theorem whose *statement* they must also read, and until now nothing measured those statements — the same hole `StatementGate.lean` closed one level down. The instrument is `#assert_statement_surface_delta`, which reports the **difference** against a baseline headline (a gate about the headline shares nearly all of its surface; only the difference informs), plus `_contains`/`_omits` for shape claims. Negative controls: `probes/StatementSurfaceProbe.lean` §§6–9, including the subtle one — `_omits` must consult the **raw** closure, or an absence claim could be satisfied through a generated companion the report filter hides. **`lake build` now carries nine obligations** (the ninth is risk S10's, added 2026-08-09). Two findings, AZ and BA, in their own rows. |

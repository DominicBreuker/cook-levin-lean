...
| **encoding-honesty audit (S5)** | ⚠ **audited 2026-07-30-c, partly STRUCTURAL 2026-08-01.** Tail: a real `Serialize`-backed parser. Head: the hypothesis-side hole (FINDING AN/AO) is closed by *restricting* the statement — `CookLevinHonest.CookLevinStr : NPcompleteStr SAT` is the one to quote. Verdicts 1–13 below; evidence `probes/HonestyAuditProbe.lean` §§1–8. |
...
| 13 | the *hypothesis* side of `NPhardStr` (**the statement to quote**) | ✅ **honest, structurally.** `InNPWitnessStr` fixes the input type to `List Bool` and the layout to `certState`, so there is no `encX` field to choose: the composite's encode is the closed formula `certState x ++ [1^(size x)]` (`HonestyAuditProbe` §8, `rfl`), and the verifier must decide from the raw string. `CookLevinHonest.CookLevinStr : NPcompleteStr SAT`, axiom-clean, derived from `CookLevin''` in one line. |

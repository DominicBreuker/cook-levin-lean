...
| `#print axioms Complexity.Lang.Cmd.polyCost_of_costSafe` | `[propext, Classical.choice, Quot.sound]` — **the cost ladder has an architecture** (2026-07-28-c, `Lang/CostPoly.lean`, sorry-free): `Cmd.PolyCost`, a compositional `∃ K D, cost ≤ K·(cap+1)^(D+1)` closed under `seq` / `ifBit` / cost-safe `forBnd`, with the `decide`-able `Cmd.CostSafe` and the growth rules for self-referential loops; `Cmd.get_length_eval_le` (per-register growth ≤ cost) is what makes `seq` compose. `S1Witness.S1CostBound` replaces the fixed degree-10 contract by an existential polynomial, so `S1Witness.s1Program_costLeSize` is now the last S1 `sorry` and is **purely structural**. Measured (`probes/S1CostSafeProbe.lean`): 4.0% of the program's 116825 loops are not cost-safe, in three shapes; FINDING X: the `copy r r` no-op costs `\|r\|+1` and is used on `EOUT_C` |
| `#print axioms Complexity.Lang.Cmd.capCost_of_capChk` | `[propext, Classical.choice, Quot.sound]` — **a cost predicate that survives a loop** (2026-07-29, `Lang/CostGrow.lean`, sorry-free). ⚠ **FINDING Z**: a cost predicate with ONE cap (`Cmd.PolyCost` over `costReads`) cannot survive a `forBnd` — the body's outputs are re-capped at `poly(M)` each iteration, so `m` iterations give a **tower**; that is why `polyCost_forBnd` must forbid the body from writing what it cost-reads, and why the 4% residual existed. `Cmd.CapCost c F F'` uses **two** caps (a frozen `MF`, a global `N`): `cost ≤ K·(MF+1)^D·(N+1)`, growth `≤ N + K·(MF+1)^D`, `F'` still `≤ MF + K·(MF+1)^D`. Cost linear in `N` pays for **FINDING X** (`copy EOUT_C EOUT_C`) for free; growth independent of `N` is what stops compounding. With `Cmd.NoGrow` (idempotent, so a body may be iterated freely — the drained cursor), `Cmd.GrowOk` and `Cmd.promote` (a written register keeps its cap when `GrowOk` certifies it against the strictly frozen set), plus the decidable `Cmd.capChk` and the one-liner `Cmd.costLeSize_of_capChk`. Measured (`probes/S1GrowSafeProbe.lean`): accepts every stage including `S1CardEmit.cFive`; over the whole program rejects **exactly two loops**, both in `S1StepLoop.scanSeen` (the dedup scan bounded by the accumulator `SSEEN`). Two open sub-items, both specified in the HANDOFF: a **merged flow-sensitive growth pass** (`GrowOk` is deliberately flow-insensitive and cannot see that `SAX` is capped at `concat SSEEN SAX SSEEN`), and a **bitmask/hoisting rewrite of the checker** (`cPrelude` alone does not finish in 10 min). |
| Genuine `sorry`s (Group C) | **6 in built code** (unchanged): 2 on the live path (`red_inNP`'s `inTimePoly` half, `hasDeciderClassical`), 3 in dead code (`MultiToSingle`), and 1 cost obligation (`S1Witness.s1Program_costLeSize`) — **the only `sorry` between the current state and an axiom-clean `NPhard'' SAT`**. **`Simulators/CookTableau.lean`/`GuessTableau.lean` are fully `sorry`-free** — `cookTableau_correct`, `guessTableau_correct`, and **both size bounds** (`≤ (2·(n+1))^10`, 2026-07-24) all sorry-free & axiom-clean |
...
- **S1 is feasible but expensive — and its target is now TRUE and decomposed
  ...
  **the per-entry preamble, the entry loop and the `stageC` assembly landed
  ...
  whole-program cost ladder `S1Witness.s1Program_costLeSize` is the only S1
  obligation left.**
...
| **S1** | **if-on-the-answer** `FlatSingleTMGenNP ⪯p FlatTCC` (all-zeros tableau, never simulates `M`). Deepest unsoundness. | `Reductions/FlatSingleTMGenNP_to_FlatTCC.lean` | **Down to ONE program + THREE contracts (2026-07-27).** Tableau mathematics, both size bounds, the map, the guard, the correctness iff, six of seven program stages, five of stage C's seven card families and both head seams are all built and axiom-clean; `FrontS1Comp.SAT_NPhard''_of_S1` states the remaining obligation `sorry`-free. The prelude family's emitter landed 2026-07-27-c. The prelude family's emitter landed 2026-07-27-c and `stepBlocks`'s entry body 2026-07-28. What is left: the per-entry preamble (nine numbers off `PTRANS`), the entry loop (`S1Step.emitFold_run` + `stepSummand_fold`), the three-line `stageC` assembly, and `S1Witness.s1Program_costLeSize`. |
...
| **S0** | **hardness reduction reaches a `sorry`** — `NPhard_GenNP` relies on `hasDeciderClassical` (`sorry`). Its second defect (the vacuous `fun _ => 0` size bound) is **fixed** — Part 0.1, 2026-07-04-b: the bound is now the honest `certBound n + timeBound (n + certBound n) + 3`. | `GenNP_is_hard.lean` | Remaining half closes with C8. ⚠ C8 scoping (2026-07-04): the sorry is *classically closable* by the cheating encoder — do NOT close it that way; it is the honest marker of the open hardness half until `NPhard''` lands (HANDOFF F1). |

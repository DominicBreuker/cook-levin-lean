...
## Status snapshot (verified 2026-07-30)

| | |
|---|---|
| `lake build` | ✅ green (~4 min; `S1Witness.lean` alone spends ~3 min in the kernel on the cost ladder's `decide`) |
...
| `#print axioms CookLevin` | `[propext, sorryAx, Classical.choice, Quot.sound]` — the **legacy** headline, over the vacuous `⪯p`. Slated for deletion, not for proof. |
...
| Genuine `sorry`s (Group C) | **5 in built code**, all on the legacy path, all to be **deleted** with it rather than proved: 2 on the *legacy* path (`red_inNP`'s `inTimePoly` half, `hasDeciderClassical`) and 3 in dead code (`MultiToSingle`). **None is on the `NPhard''` path** — the S1 cost obligation `S1Witness.s1Program_costLeSize` was the last one and is closed (2026-07-29-b), so **`FrontS1Comp.SAT_NPhard''` is axiom-clean**. **`Simulators/CookTableau.lean`/`GuessTableau.lean` are fully `sorry`-free** — `cookTableau_correct`, `guessTableau_correct`, and **both size bounds** (`≤ (2·(n+1))^10`, 2026-07-24) all sorry-free & axiom-clean |
...
| Remaining to a real proof | **none.** The honest theorem is proven. What is left is the honesty audit (S5) + the legacy deletion — ~2–3 sessions. |
...
⚠ **Destination A is REACHED** (2026-07-30-b) on the honest statement
`NPcomplete''`. The steps below are kept as the finding log that got there; the
live plan is in [`HANDOFF.md`](HANDOFF.md) and is exactly two items: the
**honesty audit (S5)** and the **legacy-front deletion**. Destination B, the
...
|---|-----|----------|--------------|
| **S3** | `⪯p` bounds **output size only** — the enabling weakness that lets S1/S2 typecheck and makes `NPcomplete` too weak to be faithful. | `NP.lean`, `Lang/PolyTime.lean` | ✅ **Superseded, not migrated.** The honest line `⪯p'`/`NPhard''`/`NPcomplete''` is built end to end and `CookLevin''` quotes it; there is deliberately **no** `NPcomplete'' → NPcomplete` bridge. `⪯p` survives only under the legacy front, and dies with it. Historical note: **Engine live & endgame design SETTLED.** Honest target `polyTimeComputable'`/`⪯p'` built on the free line; live chain instances `kSAT3_reducesPolyMO'` and `flatTCC_reducesPolyMO'` (first sound-tail step, 2026-07-02). The `NPhard'` transport is settled & machine-validated: `SeamData`/`comp` (Cmd-level chain composition, fully proven) + `NPhard'`/`NPcomplete'`, hardness at endpoints only. Execute via plan step 2. |
| **S1** | **if-on-the-answer** `FlatSingleTMGenNP ⪯p FlatTCC` (all-zeros tableau, never simulates `M`). Was the deepest unsoundness. | `Reductions/FlatSingleTMGenNP_to_FlatTCC.lean` | ✅ **CLOSED on the honest line (2026-07-29-b).** The tableau mathematics, both size bounds, the map, the guard, the correctness iff, all seven program stages and both head seams are built and axiom-clean; `FrontS1Comp.SAT_NPhard''` is unconditional and `sorry`-free. The *legacy* `⪯p` reduction in this file is still the dishonest one — it dies with the legacy-front deletion (plan step 2 / HANDOFF NEXT-TOP-DOWN item 2), not with more S1 work. |
| **S2** | **dummy TM bridges** — `bridgeMachine` discards `M`; predicates ignore `M`. | `LM_to_mTM.lean`, `mTM_to_singleTapeTM.lean` | ✅ **Superseded.** The honest line does not route through them (C8's per-`Q` front replaces the whole bridge stack). They are dead code on the `NPcomplete''` path and are **deleted** with the legacy front — see HANDOFF NEXT-TOP-DOWN item 2. |
| **S0** | **hardness reduction reaches a `sorry`** — `NPhard_GenNP` relies on `hasDeciderClassical` (`sorry`). Its second defect (the vacuous `fun _ => 0` size bound) is **fixed** — Part 0.1, 2026-07-04-b: the bound is now the honest `certBound n + timeBound (n + certBound n) + 3`. | `GenNP_is_hard.lean` | ✅ **Superseded (2026-07-29-b).** `FrontS1Comp.SAT_NPhard''` proves the hardness half without ever touching `hasDeciderClassical`. ⚠ The `sorry` is *classically closable* by the cheating encoder — do **NOT** close it that way. **Delete `GenNP_is_hard.lean`** with the legacy front (HANDOFF NEXT-TOP-DOWN item 2). |
| **S5** | **ENCODING HONESTY IS NOT ENFORCED — the end-to-end audit has never been done.** Each `⪯p'` witness's `eIn`/`encodeIn` must be the natural layout of its *input* (never of `gmap v`), `decodeOut` the inverse of the natural *output* layout, and all reduction work must live in the `Cmd`. The trivial dishonest instantiation satisfies **every** field. | the six composed witnesses: `FrontWitness.WQ`, `S1Witness.s1_reductionLang`, `FlatTCCFree`, `FlatCCBinFree`, `BinaryCCFSATFree`, `FSATSATFree`; plus the two verifiers and the five seams | ⚠ **OPEN, and now the ONLY soundness question.** `NPcomplete'' SAT` is machine-checked, so this is the one way it could still fail to mean what it says. Audit each witness and record a one-line verdict here. Known items to rule on: `WQ.encodeInQ`'s trailing `1^(size x)` (derived tally the front *consumes* — layout, not work); `S1CostBound.cost_bound` being an anonymous `Exists.choose` (nothing depends on it definitionally — the project builds); `satEncX`'s register 1 = `1^\|N\|` (derived, but inherited from `evalCnfDecidesLang`'s own `CLAUSE_TALLY`); and `InNPWitnessLangFreeSplit.encX_size`'s enormous slack (measured `23` vs `≥ 200000`) — that field constrains nothing and is **not** an honesty check. The five seams' `mfc`s are pure scrubs by construction (`clearRange` only clears), so they are a spot check. See HANDOFF NEXT-TOP-DOWN item 1. |
...
| **C8** | **real `NPhard_GenNP`** (`hasDeciderClassical`). | **SCOPED 2026-07-04 — verdict FEASIBLE-BUT-EXPENSIVE (~5–7 sessions, ~2–4K LOC)**, probe `probes/C8SeamProbe.lean` green (seam-targeting GO). ★ Blocking finding **F1**: the current `inNP` hypothesis is classically TRUE for every predicate (cheating `DecidesBy.encode`), so `NPhard'` over it can never be honest — the hypothesis must strengthen to a free-line verifier witness (`NPhard''` over `InNPWitnessLangFreeSplit`: Cert = `List Bool`, split pair layout, real `Cmd` verifier); owner decision **C8-0**. Also found: `FlatSingleTMGenNP` port bug (`list_ofFlatType 1` vs Coq `sig M`, missing `tapes M = 1` — F2) and the hard-coded `encodeIn_size ≤ 2n+1` blocks `W_Q` (F3). Full findings F1–F6 + build decomposition C8-0…C8-5 in HANDOFF. Subsumes S2. **C8-0 SIGNED OFF + C8-1 DONE (2026-07-04 part 2):** `NPhard''`/`NPcomplete''` over `InNPWitnessLangFreeSplit` live (`PolyTime.lean`), `FlatSingleTMGenNP` Coq-faithful, `encBound` generalization threaded, axiom profiles unchanged. **C8-2 DONE (2026-07-05):** the accept-by-halting wrapper (`Lang/AcceptHalt.lean`: `demoteHalt` + accept/reject run transports + `runFlatTM_first_halt` trajectory recovery) and the tape-format-check gadget (`Lang/FormatCheck.lean`: `formatCheckTM` + forward run/traj + stuck direction + the `certOKB` grammar ⇔ decode), plus the composition glue `composeFlatTM_stuck_M1` (TMPrimitives) — all sorry-free & axiom-clean, probe green (`probes/C8GadgetsProbe.lean`). Next: C8-3 (Cmd pieces; C8-4 assembly notes recorded in HANDOFF). |

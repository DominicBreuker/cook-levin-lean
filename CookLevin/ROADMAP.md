...
- **C2 is the linchpin — and is under-built (the May-2026 headline finding;
  current status: 5 of 12 ops proven, see the risk-register entry + HANDOFF).**
  ...
  `bitTestTM`) and a ~1.6K-LOC gadget library is sorry-free. **At the time of
  the finding:**
  - 10 of 12 `compileOp`s were `compiledCmd_default` stubs; only
    `appendOne`/`appendZero` had real TM bodies.

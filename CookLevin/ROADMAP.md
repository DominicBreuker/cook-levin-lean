...
| **C4** | **layer → framework bridge.** | ✅ Engine done (`toFrameworkWitness'`, `inNPLang`/`red_inNPLang`, `inNPLang_to_inNP`, capstones `reducesPolyMO_of_lang`/`red_inNP_of_lang`), sorry-free modulo C2. **The FREE line (`inNPLangFree`/`red_inNP_of_langFree`) now has a live, axiom-clean application** (`inNP_kSAT3_free`, 2026-07-02: concrete re-encoder `kSAT3_precomposeData` + reduction program `kSAT3_reductionLang` — the template for the sound-tail migration). ⚠ The canonical engine is unpopulatable (size-unsound product encoding); ⚠ `FreePrecomposeData`/`PolyTimeComputableLang` do not *enforce* encoding honesty (`eIn`/`decodeOut` unconstrained) — see HANDOFF key risk finding. Remaining: honest layer reductions (S1 + sound tail) + discharge C2. |

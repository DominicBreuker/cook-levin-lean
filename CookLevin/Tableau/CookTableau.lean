...
The central construction: a single-tape machine `M` accepts the string `s` within `steps`
steps iff a tableau of `steps + 1` rows of width `steps + 3` can be built row by row from
the initial row `confRow M s` by the cards `cookCards M`, ending in a row containing a
halting cell. Cells are `tCell` (tape symbol), `hCell` (head: state and symbol) or
`bCell` (boundary); a card is a window of three cells and its successor window. The
`copy*`, `halt*` and `step*` card families realise, respectively, cells away from the head,
a halted head and one transition of `M`. `relpower_of_run` and `run_of_relpower` are the
two directions of the equivalence, for configurations fitting in a row of width `n`, and
the `*_length_le`/`*_size_le` lemmas bound the size of the tableau instance.
-/

set_option autoImplicit false

namespace CookLevin.Tableau
...
/-- The key-dedup worker. **Public** so that
...
The two directions `relpower_of_run` and `run_of_relpower` are assembled below from the
following lemmas:
...
/-- **(1b) Card completeness — the inversion heart.**
...
/-! ### A constrained special case

For an immediately-halting single-tape machine on empty input, the tableau
is coverable for every step budget, matching acceptance. It exercises the boundary marker (window 0 is `(#, head, blank)`) and the

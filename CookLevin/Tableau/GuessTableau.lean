...
/-!
# The tableau with a guessed certificate

`FlatSingleTMGenNP` asks for a certificate appended to the input. `guessTableau M s maxSize
steps` extends the Cook tableau by a prelude row: the certificate region of row `0` is
filled with wildcard cells, and the prelude cards resolve them in the first step to an
arbitrary string of length at most `maxSize`, after which the deterministic cards of
`CookTableau.lean` take over. `guessTableau_correct` proves
`(∃ cert, … ∧ acceptsFlatTM M [s ++ cert] steps) ↔ FlatTCCLang (flattenTCC (guessTableau …))`,
and `guessTableau_size_bound` bounds the instance size.
-/
...
/-! ## Prelude algebra: cell injectivity, kind/resolution membership -/
...
/-! ## The prelude step (P1/P2)

Proof outline: row 0 has statically known shape (`preludeRow`); windows are

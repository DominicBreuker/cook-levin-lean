...
The alphabet is fixed at `sig = 3` to match `Compile_sig` (`0` =
delimiter, `1`/`2` = shifted bits). States:

- `0` : start (about to insert at the head),
- `1`, `2`, `3` : carry state holding value `0`, `1`, `2` respectively,
- `4` : halt.
...
theorem insertCarryTM_valid (ins : Nat) (h_ins : ins < 3) :
    validFlatTM (insertCarryTM ins) := by
  refine ⟨?_, ?_, ?_⟩
  · show (0 : Nat) < 5; decide
  · show ([false, false, false, false, true] : List Bool).length = 5; decide
  · intro e he
    simp only [insertCarryTM, insertCarryTrans, mkE, List.mem_cons, List.not_mem_nil,
      or_false] at he
    rcases he with h|h|h|h|h|h|h|h|h|h|h|h|h|h|h|h <;>
      subst h <;>
...
theorem insertCarryTM_step_nonblank (ins s y : Nat) (hs : s < 4) (hy : y < 3)
...
theorem insertCarryTM_step_blank (ins s : Nat) (hs : s < 4)
...
/-- Non-halt states `0..3` of the insert-carry machine. -/
private theorem not_halt_lt4 (ins s : Nat) (hs : s < 4) (cfg : FlatTMConfig)
...
theorem insertCarryTM_carry_run (ins : Nat) (suf : List Nat) :
    ∀ (pre : List Nat) (v : Nat), v < 3 → (∀ x ∈ suf, x < 3) →
  ...
  | nil =>
    ...
    rw [List.length_nil,
        run_succ_of_step _ _ _ 0
          (not_halt_lt4 ins (1 + v) (by omega) _ rfl)
          (insertCarryTM_step_blank ins (1 + v) (by omega) [] (pre ++ []) pre.length hge)]
    simp [runFlatTM, owed]
  | cons y suf' ih =>
    intro pre v hv hall
    have hy : y < 3 := hall y (by simp)
    have hall' : ∀ x ∈ suf', x < 3 := fun x hx => hall x (by simp [hx])
    ...
    rw [show (y :: suf').length + 1 = (suf'.length + 1) + 1 by simp,
        run_succ_of_step _ _ _ (suf'.length + 1)
          (not_halt_lt4 ins (1 + v) (by omega) _ rfl) hstep]
...
theorem insertCarryTM_run (ins : Nat) (suf pre : List Nat)
  ...
  | nil =>
    have hge : ¬ pre.length < (pre ++ ([] : List Nat)).length := by simp
    rw [List.length_nil,
        run_succ_of_step _ _ _ 0 (not_halt_lt4 ins 0 (by omega) _ rfl)
  ...
  | cons y suf' =>
    ...
    rw [show (y :: suf').length + 1 = (suf'.length + 1) + 1 by simp,
        run_succ_of_step _ _ _ (suf'.length + 1)
          (not_halt_lt4 ins 0 (by omega) _ rfl) hstep]

...
/-! # The Cmd → FlatTM compiler (Part 3.3 / 3.4 of ROADMAP)
...
## Module layout (refactor in progress — see `REFACTOR-HANDOFF.md`)

The compiler was a single 26K-line module; it is being split into a
DAG of `Compile/*` submodules so the build parallelises and edits
rebuild only the touched module. This file (`Compile.lean`) imports
them and currently still holds the upper soundness/assembly layers:
...
Still in this file (next to extract into `Compile/{OpSound,Assembly,Decider}`):
the per-op soundness contract `compileOp_sound_physical_residue`, the
`compileSeq`/`compileIfBit`/`compileForBnd` soundness lemmas, the C2 assembly
toolkit + `run_physical_residue_gen` + `Compile_run_physical_residue`, and the
WALL (`padRegsTM`/`paddedBitDecider`/`paddedCompute`).

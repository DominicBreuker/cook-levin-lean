...
/-! ## 2b. Cell-value test: `eqTestTM`

A tiny 3-state machine that reads the current cell and branches on whether it
equals a designated symbol `v`:
- If the cell is `v` → halt at state `1` ("equal").
- If the cell is any other in-range symbol → halt at state `2` ("other").

State `0` is the start; states `1` and `2` are both halting; the head does NOT
move. This is the bit-**value** read the Class-A ops `head`/`eqBit` need —
`delimTestTM` only distinguishes delimiter-vs-content, which covers both bit
values. On a `BitState` register's first tape cell (`∈ {1,2}` after `shiftReg`),
`eqTestTM 4 2` exits "equal" iff the stored bit is `1`. -/

/-- Test entry for the designated symbol `v`: stay, halt at state 1. -/
private def eqTestEqEntry (v : Nat) : FlatTMTransEntry :=
  { src_state := 0
    src_tape_vals := [some v]
    dst_state := 1
    dst_write_vals := [none]
...
/-- Test entry for any other symbol `w ≠ v`: stay, halt at state 2. -/
private def eqTestNeEntry (w : Nat) : FlatTMTransEntry :=
  { src_state := 0
    src_tape_vals := [some w]
    dst_state := 2
    dst_write_vals := [none]
    move_dirs := [TMMove.Nmove] }

/-- The cell-value test machine: 3 states, reads one cell, branches on `= v`. -/
def eqTestTM (sig v : Nat) : FlatTM where
  sig := sig
  tapes := 1
  states := 3
  trans := eqTestEqEntry v ::
    ((List.range sig).filter (fun w => decide (w ≠ v))).map eqTestNeEntry
  start := 0
  halt := [false, true, true]

def eqTestTM_exit_eq : Nat := 1
def eqTestTM_exit_ne : Nat := 2

theorem eqTestTM_valid (sig v : Nat) (h_v : v < sig) : validFlatTM (eqTestTM sig v) := by
  refine ⟨show (0 : Nat) < 3 from by decide, rfl, ?_⟩
  intro entry hentry
  rcases List.mem_cons.mp hentry with hEq | hNe
  · subst hEq
    refine ⟨show (0 : Nat) < 3 from by decide, show (1 : Nat) < 3 from by decide,
      rfl, rfl, rfl, ?_, ?_⟩
    · intro x hx; simp [eqTestEqEntry] at hx; subst hx; exact h_v
    · intro x hx; simp [eqTestEqEntry] at hx; subst hx; trivial
  · rcases List.mem_map.mp hNe with ⟨w, hw, hmk⟩
    subst hmk
    have hwlt : w < sig := List.mem_range.mp (List.mem_filter.mp hw).1
    refine ⟨show (0 : Nat) < 3 from by decide, show (2 : Nat) < 3 from by decide,
      rfl, rfl, rfl, ?_, ?_⟩
    · intro x hx; simp [eqTestNeEntry] at hx; subst hx; exact hwlt
    · intro x hx; simp [eqTestNeEntry] at hx; subst hx; trivial

theorem eqTestTM_tapes (sig v : Nat) : (eqTestTM sig v).tapes = 1 := rfl
theorem eqTestTM_start (sig v : Nat) : (eqTestTM sig v).start = 0 := rfl
theorem eqTestTM_sig (sig v : Nat) : (eqTestTM sig v).sig = sig := rfl
theorem eqTestTM_states (sig v : Nat) : (eqTestTM sig v).states = 3 := rfl

/-- On a cell equal to `v`, `eqTestTM` steps to state `1`. -/
theorem eqTestTM_step_eq (sig v : Nat)
    (left right : List Nat) (head : Nat)
    (h_head_lt : head < right.length) (h_get : right.get ⟨head, h_head_lt⟩ = v) :
    stepFlatTM (eqTestTM sig v)
        { state_idx := 0, tapes := [(left, head, right)] } =
      some { state_idx := 1, tapes := [(left, head, right)] } := by
  set cfg : FlatTMConfig := { state_idx := 0, tapes := [(left, head, right)] }
  have hSym : currentTapeSymbol (left, head, right) = some v := by
  ...
  have hSym' : cfg.tapes.map currentTapeSymbol = [some v] := by
    show [currentTapeSymbol (left, head, right)] = [some v]; rw [hSym]
  show Option.bind ((eqTestTM sig v).trans.find?
        (fun entry => entryMatchesConfig entry cfg))
      (applyTransitionEntry cfg) = _
  have hMatch : entryMatchesConfig (eqTestEqEntry v) cfg = true := by
    show ((0 : Nat) == cfg.state_idx &&
            decide (([some v] : List (Option Nat)) =
              cfg.tapes.map currentTapeSymbol)) = true
    rw [show cfg.state_idx = 0 from rfl, hSym']; simp
  rw [show (eqTestTM sig v).trans =
    eqTestEqEntry v :: ((List.range sig).filter (fun w => decide (w ≠ v))).map eqTestNeEntry from rfl,
    List.find?_cons, hMatch]
  rfl

/-- Helper: `find?` over the "other" entries returns the matching entry. -/
private theorem find_eqTestNe_match
    (cfg : FlatTMConfig) (w : Nat) (L : List Nat)
    (h_cfg_state : cfg.state_idx = 0)
    (h_cfg_tape : cfg.tapes.map currentTapeSymbol = [some w])
    (h_mem : w ∈ L) :
    (L.map eqTestNeEntry).find?
        (fun entry => entryMatchesConfig entry cfg) =
      some (eqTestNeEntry w) := by
  induction L with
  | nil => cases h_mem
  | cons u us ih =>
      show List.find? _ (eqTestNeEntry u :: us.map eqTestNeEntry) = _
      rw [List.find?_cons]
      by_cases huw : u = w
      · subst huw
        have hMatch : entryMatchesConfig (eqTestNeEntry u) cfg = true := by
          show ((0 : Nat) == cfg.state_idx &&
                  decide (([some u] : List (Option Nat)) =
                    cfg.tapes.map currentTapeSymbol)) = true
          rw [h_cfg_state, h_cfg_tape]; simp
        rw [hMatch]
      · have hNot : entryMatchesConfig (eqTestNeEntry u) cfg = false := by
          show ((0 : Nat) == cfg.state_idx &&
                  decide (([some u] : List (Option Nat)) =
                    cfg.tapes.map currentTapeSymbol)) = false
          rw [h_cfg_state, h_cfg_tape]
          have h_ne' : ([some u] : List (Option Nat)) ≠ [some w] := by
            intro h; injection h with h1 _; injection h1 with h2; exact huw h2
          simp [h_ne']
        rw [hNot]
        rcases List.mem_cons.mp h_mem with hwu | hwus
        · exact absurd hwu.symm huw
        · exact ih hwus

/-- On a cell `w ≠ v` (`w < sig`), `eqTestTM` steps to state `2`. -/
theorem eqTestTM_step_ne (sig v : Nat)
    (left right : List Nat) (head w : Nat)
    (h_head_lt : head < right.length) (h_get : right.get ⟨head, h_head_lt⟩ = w)
    (h_ne : w ≠ v) (h_lt : w < sig) :
    stepFlatTM (eqTestTM sig v)
        { state_idx := 0, tapes := [(left, head, right)] } =
      some { state_idx := 2, tapes := [(left, head, right)] } := by
  ...
  have hSym : currentTapeSymbol (left, head, right) = some w := by
    rw [currentTapeSymbol_in_range h_head_lt, h_get]
  have hSym' : cfg.tapes.map currentTapeSymbol = [some w] := by
    show [currentTapeSymbol (left, head, right)] = [some w]; rw [hSym]
  show Option.bind ((eqTestTM sig v).trans.find?
        (fun entry => entryMatchesConfig entry cfg))
      (applyTransitionEntry cfg) = _
  have hNotEq : entryMatchesConfig (eqTestEqEntry v) cfg = false := by
    show ((0 : Nat) == cfg.state_idx &&
            decide (([some v] : List (Option Nat)) =
              cfg.tapes.map currentTapeSymbol)) = false
    rw [hSym']
    have h_ne' : ([some v] : List (Option Nat)) ≠ [some w] := by
      intro h; injection h with h1 _; injection h1 with h2; exact h_ne h2.symm
    simp [h_ne']
  have hw_mem : w ∈ (List.range sig).filter (fun u => decide (u ≠ v)) := by
    rw [List.mem_filter]
    exact ⟨List.mem_range.mpr h_lt, by simp [h_ne]⟩
  have hFind := find_eqTestNe_match cfg w
    ((List.range sig).filter (fun u => decide (u ≠ v)))
    rfl hSym' hw_mem
  rw [show (eqTestTM sig v).trans =
    eqTestEqEntry v :: ((List.range sig).filter (fun w => decide (w ≠ v))).map eqTestNeEntry from rfl,
    List.find?_cons, hNotEq, hFind]
  rfl

/-- `eqTestTM` run: on a cell `= v` → state 1 in 1 step. -/
theorem eqTestTM_run_eq (sig v : Nat)
    (left right : List Nat) (head : Nat)
    (h_head_lt : head < right.length) (h_get : right.get ⟨head, h_head_lt⟩ = v) :
    runFlatTM 1 (eqTestTM sig v)
        { state_idx := 0, tapes := [(left, head, right)] } =
      some { state_idx := 1, tapes := [(left, head, right)] } := by
  show (if haltingStateReached (eqTestTM sig v)
            { state_idx := 0, tapes := [(left, head, right)] } = true then _
        else match stepFlatTM (eqTestTM sig v)
            { state_idx := 0, tapes := [(left, head, right)] } with
          | none => _
          | some cfg' => runFlatTM 0 (eqTestTM sig v) cfg') = _
  rw [show haltingStateReached (eqTestTM sig v)
        { state_idx := 0, tapes := [(left, head, right)] } = false from rfl,
      eqTestTM_step_eq sig v left right head h_head_lt h_get]
  rfl

/-- `eqTestTM` run: on a cell `w ≠ v` (`w < sig`) → state 2 in 1 step. -/
theorem eqTestTM_run_ne (sig v : Nat)
    (left right : List Nat) (head w : Nat)
    (h_head_lt : head < right.length) (h_get : right.get ⟨head, h_head_lt⟩ = w)
    (h_ne : w ≠ v) (h_lt : w < sig) :
    runFlatTM 1 (eqTestTM sig v)
  ...
  rw [show haltingStateReached (eqTestTM sig v)
        { state_idx := 0, tapes := [(left, head, right)] } = false from rfl,
      eqTestTM_step_ne sig v left right head w h_head_lt h_get h_ne h_lt]
  rfl

/-- `eqTestTM` never halts before its single step. -/
theorem eqTestTM_no_early_halt (sig v : Nat)
    (left right : List Nat) (head : Nat) :
    ∀ k, k < 1 → ∀ ck,
      runFlatTM k (eqTestTM sig v)
          { state_idx := 0, tapes := [(left, head, right)] } = some ck →
      ck.state_idx ≠ eqTestTM_exit_eq ∧
      ck.state_idx ≠ eqTestTM_exit_ne ∧
      haltingStateReached (eqTestTM sig v) ck = false := by
  intro k hk ck hck
  have hk0 : k = 0 := by omega
  subst hk0
  simp [runFlatTM] at hck; subst hck
...
/-- The halt states of `eqTestTM sig v` are exactly `1` and `2`. -/
theorem eqTestTM_halt_only (sig v : Nat) (i : Nat)
    (hi : (eqTestTM sig v).halt[i]? = some true) : i = 1 ∨ i = 2 := by
  change ([false, true, true] : List Bool)[i]? = some true at hi
  rcases i with _ | _ | _ | i <;> simp_all

/-- `eqTestTM_exit_eq` is a halt state. -/
theorem eqTestTM_exit_eq_is_halt (sig v : Nat) :
    (eqTestTM sig v).halt[eqTestTM_exit_eq]? = some true := rfl

/-- `eqTestTM_exit_ne` is a halt state. -/
theorem eqTestTM_exit_ne_is_halt (sig v : Nat) :
    (eqTestTM sig v).halt[eqTestTM_exit_ne]? = some true := rfl

...
/-! # SAT input encoding + demonstration deciders (Part 2, Phase C)

This file contains:

1. The shared SAT-input tape encoding `encodeInput : cnf × assgn →
   List Nat` used by all single-tape SAT-input deciders, plus the
   length / symbol / size-bound lemmas.
2. A worked library of *demonstration* `DecidesBy` witnesses on this
   encoding — `CnfEmpty`, `CnfEmptyAssgnEmpty`, `AssgnEmpty`,
   `CnfStartsEmpty`, the negate/iff-derived deciders,
   `CnfHasEmptyClause`, `AssgnContainsZero`. These were stepping
   stones to a hand-rolled single-tape `evalCnfTM`. They establish
   patterns (filter-range transition tables, `find?` walkers,
   scan-loops, `.negate`/`.iff` combinators) that the eventual
   multi-tape `evalCnfTM` (see `Deciders/EvalCnfTM.lean`) will reuse.

This file is **frozen** as of PART2.md v2's strategic pivot: the
real `evalCnfTM` is built compositionally in `EvalCnfTM.lean` rather
than as another monolithic flat TM here.
...
def decider : DecidesBy
  ...
  decides_pos := by
    ...
    have h_padded : runFlatTM ((encodeCnf N).length + 1 + k) TM
      ...
      runFlatTM_extend h_chain h_final_halt
    rw [h_k] at h_padded
  ...
  decides_neg := by
    ...
    | cons v vs =>
        ...
        · -- v ≥ 1: symbol is 1.
          ...
          rcases Nat.le.dest h_le with ⟨k, h_k⟩
          have h_padded :=
            runFlatTM_extend h_chain h_final_halt (k := k)
          rw [h_k] at h_padded
          refine ⟨_, ?_, h_final_halt, rfl⟩
...
noncomputable def decider : DecidesBy
  ...
  decides_pos := by
    ...
    rcases Nat.le.dest h_le with ⟨k, h_k⟩
    have h_padded := runFlatTM_extend h_chain h_final_halt (k := k)
    rw [h_k] at h_padded
    refine ⟨_, ?_, h_final_halt, rfl⟩

...
/-!
# Turing machine combinators

Sequential composition `composeFlatTM M₁ M₂ exit` (run `M₁` until it reaches `exit`, then
continue with `M₂` on the same tapes), the two-way branch `branchComposeFlatTM`, the loop
`loopTM` and the scanner `scanRightUntilTM`, each with a validity lemma and a run lemma describing the composed machine's trajectory in terms
of its components. All machines are single-tape.
-/
...
This is the load-bearing operational lemma. It lets us
...
/-- **Stuck-`M₁` transfer.** If `M₁`'s run from `cfg0`
...
/-! ## Branching composition `branchComposeFlatTM`
...
dispatch on the polarity bit (the sign byte `2` vs `3`
...
/-! ## The counted-loop combinator `loopTM`

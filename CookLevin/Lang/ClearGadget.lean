...
`clearRegionTM dst` empties register `dst` of an encoded tape by deleting its first cell
until the delimiter is reached, rewinding the head to cell `0` after every deletion. The
run lemmas describe the machine's trajectory on encoded tapes.
-/

set_option autoImplicit false

namespace CookLevin.Lang.ClearGadget

open TMPrimitives
open CookLevin.Lang.ScanLeft
open CookLevin.Lang.ScanPast
open CookLevin.Lang.ShiftTape
open CookLevin.Lang.AppendGadget
open CookLevin.Lang.Navigate
...
end CookLevin.Lang.ClearGadget

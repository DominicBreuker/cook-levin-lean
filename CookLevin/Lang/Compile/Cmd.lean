...
`compileOp` dispatches to the machine of each primitive operation, `compileSeq` composes
machines sequentially, `compileIfBit` tests a bit and branches, and the `forBnd` machines
iterate a body a number of times read off a register. `compileCmd` assembles them into
`Compile c` with its designated exit state `Compile.exit c`; `Compile_valid`, `_tapes` and
`_sig` are its structural properties.
-/
...
The structural invariants are kept minimal; the run lemmas of the compiled machines
carry the trajectory information the composition needs. -/
...
**Two halt states.** `branchComposeFlatTM`'s halt vector
...
)The two NEG leaves (inner / delim) are
...
`CompiledCmd` built by `compileSeq` from the op gadgets. One iteration
...
`|K2| + body.cost + 1`) at the *machine* level.

**The loop body machine** wraps this chain behind the
...
`State.get s (sb+1) = replicate i 1`). -/
...
slot), never triggered on a terminator-free residue. See `forBndIterate_run` for the
per-iteration contract this body wraps. -/
...
at **static scratch base `sb`**.

**The machine design (for the W-invariant ① and the
...
the same gadget the `copy` op needs. A `moveRegionTM`-based
...
`sb` is the compiler's static scratch-register assignment: every `forBnd` node
...
/-- The compiled machine is valid: by the per-constructor validity of each helper and
the combinator-validity lemmas (`composeFlatTM_valid`, `branchComposeFlatTM_valid`). -/

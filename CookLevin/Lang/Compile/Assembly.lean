...
/-!
# Compiler soundness for branches, loops and whole programs

The `bitTestTM` tester, the invariants that keep register lists bit-shaped
(`Cmd.eval_preserves_BitState`), the soundness of `ifBit` and `forBnd`, the program-level
contract `Compile_run_physical_residue` (the compiled machine of `c` started on the encoding
of `s` halts on the encoding of `Cmd.eval c s` within a step budget linear in `Cmd.cost`),
and the decider machine `bitDeciderTM` with `bitDecider_run`.
-/
...
/-! ## The tape-to-state bit-test gadget
...
This gadget and its run lemmas depend only on the encoding format. -/
...
(they were downstream in PolyTime.lean). -/
...
/-! ## The residue induction

The four per-fragment physical-residue contracts (op / seq / ifBit / forBnd) compose into
`Compile_run_physical_residue` by induction on `Cmd`. -/
...
The quadratic `Compile.overhead (·+1)²` fails this: summing
...
**Why 8 units per cost item, not 1.** The `forBnd` machine must do per-iteration *bookkeeping* the layer
...
`Compile.run_physical_residue_gen` close. -/
...
`physStepBudget_mono` this is the interface the budget restatement feeds to
...
/-- **Residue-tolerant `compileIfBit` contract.**
...
The `+3 ≤` one extra `physStepBudget` unit, so the
...
theorem compileIfBit_sound_physical_residue
    ...
    -- `ht`/`hG`: the tester must physically navigate to
  ...
  -- The tester is REAL now: navigate + read +
...
key accounting claim) and a cubic step budget. Discharged from the four op
...
The loop induction threads the
...
swapped to `forBndContentTM`. The **iteration contract** composes `navigateAndTestTM_run_content` → the content branch
...
for the machine and the W-invariant/budget accounting.
...
- `hbody`: the body contract at scratch base `sb + 2`, quantified over every
  ...
  fold-state sizes grow, so a single fixed bound would be wrong). -/
...
- `op o`: `compileOp_sound_physical_residue` (`hbnd` from `Op.inBounds_of_UsesBelow`);
  ① per-op from the residue formula; ② from `9·L²+9·L+30`, `L ≤ G`.
...
The `op`/`seq` cases are the structural heart; they reduce to the combinators above. -/
theorem Compile.run_physical_residue_gen (c : Cmd) (k : Nat) (s : State)
  ...
  | op o =>
      ...
      -- the op's scratch base is `k`; `k`, `k+1` exist (`+2` padding)
      ...
      · -- ② budget: `(9·L²+9·L+30)·(cost+1) ≤ physStepBudget G (Op.cost o s)`, since
        ...
        -- timeouts on products of two-atom sums.
...
/-- **The residue-tolerant physical compiler contract** — the `res0 = []` instance of
...
/-- Running `bitDeciderTM c` on `encodeTape s` halts, within
...
contract of `Compile c` with the gadget run lemma, via `composeFlatTM_run`. The

...
/-!
# Machines for the primitive operations

The `FlatTM` built for every primitive operation of the register language (append, clear,
copy, tail, head, nonEmpty, eqBit, concat) together with its structural lemmas (validity,
alphabet, tape count, exit and halt states). The run lemmas are in the `Run*` modules.
-/
...
register. The design is the **in-place marking/cursor read**:
...
`opCopy` assembly with one extra branch stage. -/
...
/-! ### `eqBit` machine definitions and shape lemmas -/
...
/-- Compile `Op.eqBit dst src1 src2`: the `joinTwoHalts`-merged branch
...
/-- Compile `Op.eqBit dst src1 src2` at scratch base `sb` (no-grow).

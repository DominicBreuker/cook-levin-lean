...
/-!
# Run lemmas: copy and tail

Run lemmas for the register copy machine (`opCopy`) and for the tail machine (`opTail`).
-/
...
/-! ### Cursor-copy run lemmas

The lemma stack for the cursor-copy machine
: step lemmas for the two custom machines, the
...
the per-op exact-residue lemma `opCopy_run` consumed by the contract case. -/
...
combinator's tight W-invariant needs. -/
...
the branch stage, and the per-op assemblies. -/

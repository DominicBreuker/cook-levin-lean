...
/-!
# Padded deciders and computers

A compiled program expects its state to contain at least `k` registers. `padRegsTM` appends
empty registers to the tape until that holds; `paddedBitDeciderTM c k` runs `Compile c` on
the padded tape and halts in one of two states according to the answer bit in register `0`
(`paddedBitDecider_run`), and `paddedComputeTM c k` halts on the encoding of the program's
result (`paddedCompute_run`).
-/
...
/-! ## Runtime register-width padding

`Compile_run_physical_residue` requires `k ≤ s.length` (its per-op gadgets
...
below is proved from this interface. -/
...
/-! #### Helpers for the padding gadget proofs -/
...
exactly `2·|encodeTape s| + 7` steps. -/
...
/-- **Empty-register padding machine.** `padRegsTM k` is
...
/-- **The padded decider run.** Runs `paddedBitDeciderTM c k` on the **narrow** input
...
answer/cost from the wide state back to `s`.

`hwle : s.length ≤ k` is the scratch-emptiness side of the scratch interface: the compiler's scratch block sits at registers `[k, k + 2·c.loopDepth)`,
...
/-! ## The padded compute run

The reduction side (`PolyTimeComputableLang.toFrameworkWitness` / `ComputesBy`) faces
the same obstacle as the decider side: `Compile_run_physical_residue` carries
...
This is the function-computation analogue of `Compile.paddedBitDecider_run`, proved from
the same `padRegsTM` interface and `Compile_run_physical_residue`. -/

...
/-!
# Run lemmas: append and clear

Run lemmas for the two append operations, the residue-tolerant tape toolkit shared by all
run lemmas, and the run lemma `clearRegionTM_run` of the clear operation.
-/
...
/-! ### Soundness of `appendOne`/`appendZero`

The two append operations run within the linear budget `2 · (encodeTape s).length + 3`,
by `AppendGadget.appendAt_run_steps` and `appendAt_steps_le`. The leading sentinel of
`encodeTape` is folded into the first marker-free block so the gadget runs from head `0`.
The live per-op contract is the residue-tolerant `compileOp_sound_physical_residue`
(append cases discharged by `Compile.opAppendBit_physical_residue`). -/

/-! ### Composition under the physical per-`Op` contract

A `decodeTape`-equality contract cannot feed
...
`M₂`'s contract plugs straight into `composeFlatTM_run`'s `h_run2`. The budget is
additive. -/
...
/-! ### Physical-contract restated composition lemmas
...
/-! ### `clear` run lemma — reusable building blocks
...
/-- **Delete-branch core: `stepDeleteRewindRawTM` run.** From
...
/-- **Clear loop body — delete branch.** When register `dst` is
...
over `navigateAndTestTM_run_content` and `stepDeleteRewind_run`. -/
...
/-- **Clear loop body — done branch.** When register `dst` is
...
over `navigateAndTestTM_run_delim` and `rewindToStart_run`
...
/-- **Residue-tolerant per-op physical contract for the append op.** The rewinding append op `opAppendBitRewind
...
/-- **Per-op contract budget loosening `9 → 54`.** The proven ops establish the tight
`(9·L²+9·L+30)·c`; `compileOp_sound_physical_residue` states the looser
`(54·L²+54·L+180)·c`, which the `eqBit` gadget's loop needs. The looser constant is free
against `physStepBudget` (`54 ≤ 72`). -/
...
/-- **`clearRegionTM` run.** Assembled from `loopTM_run`. The

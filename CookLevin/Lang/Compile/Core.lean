...
/-!
# Compiled commands and generic combinators

`CompiledCmd` packages a `FlatTM` with its designated exit state and validity bookkeeping.
`Compile.joinTwoHalts` merges two halt states into one, and `Compile.rewindBracket` wraps a
machine with the head rewind to cell `0` that makes it a single-exit compiled command.
-/

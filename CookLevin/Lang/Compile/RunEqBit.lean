...
/-!
# Run lemmas: equality of registers

Run lemmas for the register comparison machine (`opEqBitNG_run`), a loop consuming both
registers in step.
-/
...
/-! ### `eqBit` consume-loop body — the ITERATE machine

The `eqBit` gadget compares two scratch copies by a `loopTM` whose body
...
lemma below is the body's ITERATE leaf. -/
...
/-- **Step-bound helper.** The preceding-register-blocks prefix of
...
. `eqVerdictM` is the clean 2-exit
...
2-exit `{NEQ, EQ}`. Reused by the `eqBit` wrapper. -/
...
/-! ### `testMachine` — the consume-loop body decision
...
/-! ### `compareBodyTM` — the `eqBit` consume-loop body
...
/-! ### The consume-loop abstract semantics + State iteration
...
verdict (`eqVerdictM`) tests; the verdict assembly consumes it. -/
...
/-- **Loop tape-length invariance.** Within the matched prefix
...
/-! ### `compareLoopTM` — the `eqBit` consume loop
...
/-- **`eqBit` budget arithmetic.** The tester
...
The two branches merge through `joinTwoHalts`. The step budget is stated separately. -/

...
/-!
# Scan left to a marker

`scanLeftUntilTM sig target` moves the head left over a `target`-free block and halts on
the first `target`. With the leading marker `3` of `Compile.encodeTape` this rewinds the head
to cell `0`, which every compiled operation does before it exits (`rewindTwoPhase_run`).
-/

set_option autoImplicit false

namespace CookLevin.Lang.ScanLeft
...
The append/insert gadget (`AppendGadget.appendAtTM`) exits with its head on the
**last** tape cell, i.e. *on* the trailing terminator `endMark = 3` (`insertCarryTM_run`).
...
/-! ### The two-phase rewind (`rewindTwoPhaseTM`)

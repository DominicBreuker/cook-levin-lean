...
/-!
# Frame and locality of program evaluation

`Cmd.UsesBelow k c` says the program `c` only touches registers below `k`. Such a program
leaves the registers at or above `k` unchanged (`Cmd.eval_get_frame`) and its result and
cost depend only on the registers below `k` (`Cmd.eval_agree`, `Cmd.cost_agree`). Also the
evaluation of loops as a fold (`Cmd.eval_forBnd`) and register-length bounds.
-/

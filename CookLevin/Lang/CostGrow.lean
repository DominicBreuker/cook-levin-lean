...
/-!
# A decidable polynomial cost certificate

`Cmd.CapCost c F MF N K D` bounds the cost of `c` by `K · (MF + 1)^D · (N + 1)` whenever the
registers in the frozen set `F` have length at most `MF` and all registers have length at
most `N`; a loop only needs its body to leave the frozen registers alone (`Cmd.NoGrow`), so
the bound survives iteration. `Cmd.chk` is a decidable pass over the syntax that checks
this, `Cmd.chk_sound` proves it, and `Cmd.costLeSize_of_chk` turns a passed check into the
polynomial bound `cost ≤ K · (size + 1)^D` on the state size.
-/
...
/-! ## `CapCost`, the two-cap predicate -/

/-- **The two-cap polynomial cost bound.** `F` is capped by `MF`, everything by
`N`. The cost may be linear in `N`; the *growth* may not, which is what keeps a loop
...
a huge output register — is paid for by the `(N+1)` factor. -/
...
/-! ## the bridge to a witness's `cost_le`

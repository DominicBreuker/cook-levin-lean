...
/-!
# Scan past a delimiter

`scanPastDelimTM sig target` moves the head right over a `target`-free block and halts one
cell past the first `target`. The per-operation machines use it to step from one register
to the next.
-/

set_option autoImplicit false

namespace CookLevin.Lang.ScanPast

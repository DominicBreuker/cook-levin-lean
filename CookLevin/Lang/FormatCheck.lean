...
`formatCheckTM w` scans the whole tape and halts in state `w + 6` with the head at cell `0`
if the tape has the shape `3 ({1,2}* 0)^(w+1) 3` of an encoded state with `w + 1` registers,
and gets stuck otherwise (`formatCheck_halting_iff`). It guards the front machine, whose
certificate is an arbitrary string over the machine alphabet.
-/

set_option autoImplicit false

namespace CookLevin.Lang.FormatCheck
...
end CookLevin.Lang.FormatCheck

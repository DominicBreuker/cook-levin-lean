...
/-!
# Insert and delete one cell

`insertCarryTM` inserts a symbol at the head position, shifting the rest of the tape right
by one cell while carrying the displaced symbol in the state; `deleteCarryTM` deletes the
cell under the head, shifting the rest left. Both work over the four-symbol alphabet of the
tape encoding and are the building blocks of the register operations that change a
register's length.
-/

set_option autoImplicit false

namespace CookLevin.Lang.ShiftTape
...
owed value (appending one cell) and halt. -/
...
/-- **The shared "insert one symbol" run lemma.** Starting in

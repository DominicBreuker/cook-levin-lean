...
/-!
# Cost bounds for loop-free program fragments

`Cmd.cost_le_flat`: the cost of a loop-free fragment is at most `flatK · (M + 1)` when every
register it reads has length at most `M`. `Cmd.cost_forBnd_flat_le` and the `cost_*Loop_le`
lemmas turn such a body bound into a bound for a loop.
-/
...
Two facts about `Cmd.eval`/`Cmd.cost` alone: `Cmd.get_length_eval_le` bounds the growth
of one register by the cost, and `Cmd.forBnd_counter_le` is what an outer loop needs to

...
/-!
# The covering-card problem

`CCLang` for tableaux over a finite alphabet `Fin k`, and its flattened version `FlatCCLang`
over `Nat` together with the flattening and unflattening maps and their inverse lemmas.
-/

...
/-!
# The covering-card problem with wildcards

`TCCLang` and its flattened version `FlatTCCLang`: as `CCLang`, but a card may contain
wildcard cells matching any symbol. This is the target of the tableau reduction.
-/

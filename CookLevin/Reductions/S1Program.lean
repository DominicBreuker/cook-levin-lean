...
/-!
# Tableau reduction: the program

The output layout `s1Key`/`s1Extract` (registers `SIGMA`, `INIT`, `CARDS`, `FINAL`,
`STEPS`), the card stage `stageC`, the output stage `stageMYes`, and the whole program
`s1Program` with its correctness `s1Program_computes`: on the input layout
`HeadLayout.headEncodeIn i` it produces the layout of `S1Map.s1Map i`.
-/
...
open CookLevin.Lang CookLevin.Tableau HeadLayout

/-! ## The output key (= `FlatTCCFree.encodeIn` registers 1–5)

These are program-layout definitions. -/
...
abbrev EScratch (r : Var) : Prop :=
  (37 ≤ r ∧ r < 48) ∨ r = NumGadgets.HEAD ∨ r = NumGadgets.INBLK
    ∨ r = NumGadgets.SKIPR
...
theorem stageInit_frame (M : flatTM) (str : List Nat) (maxSize steps : Nat) (s : State)
  ...
  (S1Emit.stageInit_run M str maxSize steps s hsig hst hS hmx hsp hb).2 r
    ⟨hO, ne_of_not_scratch hd S1Emit.EC (by decide),
      ...
      ne_of_not_scratch hd S1Emit.EJ3 (by decide),
      ne_of_not_scratch hd S1Emit.EK1 (by decide),
      ne_of_not_scratch hd NumGadgets.HEAD (by decide),
      ne_of_not_scratch hd NumGadgets.INBLK (by decide),
      ne_of_not_scratch hd NumGadgets.SKIPR (by decide)⟩
    (ne_of_not_scratch hd S1Emit.EA (by decide))
    (ne_of_not_scratch hd S1Emit.EB (by decide))
    (ne_of_not_scratch hd S1Emit.ESG (by decide))

/-! ## Stage C — the card emitter

Its target is `S1Cards.encCards_eq`:
...
/-! ### The five families of `cFive` fit inside `CDirty`
...
`hstart` is needed because `S1Cards.preludeBlocks` is indexed by `min M.start M.states`,
so the emitter reads register `PSTART`; stage P produces it and stages Σ / I preserve
it. -/
...
/-! ## Stage M-yes — the output multiplex
...
/-! ### The guard-false half of `computes`
...
/-- **The guard-false branch of `s1Program` is correct.** -/

...
/-!
# BinaryCC to FSAT: the map

`BinaryCC_to_FSAT_instance C` is the Boolean formula whose variables describe the cells of
a tableau of `steps + 1` rows and which says that row `0` is the initial row, every window
of consecutive rows is covered by a card and the last row contains a final substring.
`BinaryCC_to_FSAT_instance_correct` proves `BinaryCCLang C ↔ FSAT (…)` and
`BinaryCC_to_FSAT_instance_size_bound` bounds its size polynomially.
-/

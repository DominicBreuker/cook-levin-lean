...
/-!
# BinaryCC to FSAT: encodings

The serialization `serF` of Boolean formulas into bits (prefix notation, unary variable
indices) and the input register layout of the program computing the tableau formula, with
the size relations between a formula and its serialization.
-/
...
/-! ## 2. The input/output register layout
...
`encodeIn` below is pinned to that frame so the seam

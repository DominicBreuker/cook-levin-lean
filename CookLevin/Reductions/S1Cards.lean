...
/-!
# Tableau reduction: the card list as a stream

`GuessTableau.guessCards M` is defined with `Fin`-typed ranges and filters. This module
restates the flattened card list as nested `flatMap`s over `List.range` with cell codes
given by arithmetic on the numbers parsed from the machine (`cardBlocks`), and proves the
restatement equal to the flattened original. That is the form a program can emit.
-/
...
equation holds for malformed entries too. -/

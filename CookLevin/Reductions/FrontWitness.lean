...
For every verifier witness `W : NPWitness Q`, the program witness `WQ W …` computes the
front instance `fQ` from the input layout `W.encX`. Its `computes`, `cost` and size fields
are proved here from `FrontProgram.lean`; together with `fQ_correct` this gives the
reduction of `Q` to the generic problem.
-/

set_option autoImplicit false

namespace CookLevin.Lang.FrontWitness

open CookLevin.Lang
open CookLevin.Lang.FrontMachine
open CookLevin.Lang.FrontLifting
open FrontPieces
...
`width_le`/`decode_agree` inputs of the witness. -/
...
/-- **`frontProgram` touches only registers `< B + 10`**. -/
...
/-- **The reduction's input layout: `encX`, and nothing else.** Under `NPhard` it is
`certState`, the raw input string. The unary size register is computed on-machine by
`FrontPieces.tallyCells` (see `FrontProgram`). -/
def encodeInQ (W : NPWitness Q) (x : X) : State := W.encX x

/-- The per-`Q` front machine (a constant). -/
def MmachineQ (W : NPWitness Q) : flatTM :=
  MQ W.verifier.c W.verifier.regBound W.xWidth

/-- The reg-1 machine constant — `encSyms (flattenTM M_Q)`. -/
def MconstQ (W : NPWitness Q) : List Nat :=
...
def MstepF (W : NPWitness Q) (cs ks ds : Nat) (x : X) : Nat :=
  cs * (State.size (W.encX x) + 1) ^ ks + ds

/-- **The reduction's output decoder.** The machine is the per-`Q` constant
`M_Q` (reg 1 holds `encSyms (flattenTM M_Q)`); reg 2 through the
`encSyms` inverse; regs 3/4 by length. -/
noncomputable def decodeOutQ (W : NPWitness Q)
...
def cQ (W : NPWitness Q) (cm km dm cs ks ds : Nat) : Cmd :=
  frontProgram (MconstQ W) W.xWidth (BwidthQ W) cm km dm cs ks ds

theorem BwidthQ_ge5 (W : NPWitness Q) : 5 ≤ BwidthQ W := by
  unfold BwidthQ
  have : headRegBound ≤ max headRegBound (W.xWidth + 1) := le_max_left _ _
  simpa [headRegBound] using this

theorem xWidth_lt_BwidthQ (W : NPWitness Q) : W.xWidth < BwidthQ W :=
...
theorem encodeInQ_tally (W : NPWitness Q) (x : X) :
...
theorem computesQ (W : NPWitness Q) (cm km dm cs ks ds : Nat) (x : X) :
...
theorem encodeInQ_size_le (W : NPWitness Q) (x : X) :
...
theorem encodeInQ_width (W : NPWitness Q) (x : X) :
...
theorem decodeOutQ_agree (W : NPWitness Q) (cm km dm cs ks ds : Nat)
    (x : X) (m : Nat) :
...
against a single-summand polynomial `costBoundQ`. The program cost
...
theorem map_range_encX (W : NPWitness Q) (x : X) :
...
reduction program's cost and the front instance's output size. Every monomial term is
evaluated at `W.dBound n`, not at `n`: the
...
noncomputable def costBoundQ (W : NPWitness Q) (cm km dm cs ks ds : Nat) :
    Nat → Nat := fun n =>
...
theorem costBoundQ_poly (W : NPWitness Q) (cm km dm cs ks ds : Nat) :
...
theorem costBoundQ_mono (W : NPWitness Q) (cm km dm cs ks ds : Nat) :
...
theorem cQ_cost_le (W : NPWitness Q) (cm km dm cs ks ds : Nat) (x : X) :
...
theorem fQ_output_size_le (W : NPWitness Q) (cm km dm cs ks ds : Nat) (x : X) :
...
noncomputable def WQ (W : NPWitness Q) (cm km dm cs ks ds : Nat) :
...
Step 2 needs a lower bound on the tally, which is what the `sizeLB` field provides. -/
theorem exists_front_constants (W : NPWitness Q) :
...
end CookLevin.Lang.FrontWitness

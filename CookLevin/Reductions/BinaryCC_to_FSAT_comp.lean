...
Composes the witness of `FlatTCC → BinaryCC` with the witness of `BinaryCC → FSAT` into one
program witness `flatTCC_to_FSAT_witness`, connected by a small scrub program that moves
the first witness's output into the second's input layout.
-/
...
/-- **The seam.** The bridge pushes the previous seam's bridge
...
`hkey` is split with `simp only [List.cons.injEq, and_true]` rather than `injection`,
which would symbolically execute the reduction programs. -/

...
Composes the witness of `FlatTCC → FlatCC` with the witness of `FlatCC → BinaryCC` into one
program witness `flatTCC_to_binaryCC_witness`.
-/
...
/-- **The seam.** The bridge is a `cardConvert_run` frame argument:
...
/-- **The composed witness**: the whole

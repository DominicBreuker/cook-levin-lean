...
/-!
# Tableau reduction: the prelude cards, model

`GuessTableau.preludeCards` restated as the nesting `preludeSeg` of loops over kinds and
resolutions that a program can implement, with the equality to the original.
-/
...
is dead once the preamble has finished). There is no head-room left

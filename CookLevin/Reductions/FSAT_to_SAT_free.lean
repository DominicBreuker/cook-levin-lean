...
/-!
# FSAT to SAT: the cost bound and the witness

The cost bound `buildSAT_cost_le` of the program of `FSAT_to_SAT_free_run.lean` and the
resulting program witness `fsatSAT_reductionLang` for the map `preTseytin`.
-/
...
The growing-buffer worry is a NON-ISSUE: within one
...
`gad_le` absorbs the growth *inside* the gadget. Perf discipline: loop frame facts precomputed as `private` one-liners
...
/-- **The per-token cost ceiling**: with the

...
/-!
# The reduction FlatSingleTMGenNP → FlatTCC

`s1Map (M, s, maxSize, steps)` is `flattenTCC (guessTableau M s maxSize steps)` if `M` is
a valid single-tape machine and `s` a string over its alphabet, and a fixed non-instance
otherwise. `s1Map_correct` proves `FlatSingleTMGenNP i ↔ FlatTCCLang (s1Map i)` from
`GuessTableau.guessTableau_correct`, and `s1Map_size_le` bounds the output size.
-/
...
/-- **The reduction map.** A computable function of the
...
instance's `encodable.size`. This needs the `encodable FlatTM` instance measuring every data field
: under the former flat-`5`-per-entry measure `M.trans.length` was

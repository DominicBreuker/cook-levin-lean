...
The program that writes the four registers of the front instance `fQ x` (the constant
machine `M_Q`, the encoded input, and the unary bounds `maxSize x` and `steps x`) in the
layout `HeadLayout.headEncodeIn`, with its run lemma `frontProgram_run` and cost bound
`frontProgram_cost_le`.
-/
...
This is the `computes` crux of the front witness. -/

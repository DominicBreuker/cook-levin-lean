...
/-!
# Tableau reduction: the emitter and the Σ, F, I stages

`emitBlk` appends one unary block to an output register. The stages write the alphabet
size, the final strings and the initial row of the target instance.
-/
...
it is paid once, outside every loop. The
...
/-- The registers stage I's loops may write (`NumGadgets.readNum`'s three
...
private theorem readItem_frame (s : State) (r : Var) (hr : IClean r) :
  ...
  simp [S1Parse.readItem, NumGadgets.readNum, NumGadgets.cSkip, Cmd.writes,
...
theorem stageInit_usesBelow : Cmd.UsesBelow stageInit 48 := by
  simp [stageInit, iniPre, iniMid, iniBodyA, iniCellA, iniCellK, repOne, enop,
    ...
    NumGadgets.readNum,
    NumGadgets.cSkip, Cmd.UsesBelow, Op.UsesBelow,

...
`MQ c k w` is the single-tape machine that checks the tape format for `w + 1` registers
(`formatCheckTM`), runs the compiled verifier `c` on `k` padded registers and halts exactly
when the verifier accepts (`demoteHalt`). `MQ_accepts_of_accept` and
`MQ_no_reject_of_accepts` relate its acceptance by halting to the verifier's answer bit.
-/

set_option autoImplicit false

namespace CookLevin.Lang.FrontMachine

open CookLevin.Lang.AcceptHalt
open CookLevin.Lang.FormatCheck
...
end CookLevin.Lang.FrontMachine

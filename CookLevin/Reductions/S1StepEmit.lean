...
/-!
# Tableau reduction: step cards of one transition

`stepEmit` emits the cards of one normalised transition entry from the registers holding
its numbers, with `stepEmit_run`; `emitFold_run` is the run lemma of a loop of emitters.
-/
...
/-- `1^((σ+1)(q+1))` — the source state's head-cell base. -/
def TQ  : Var := 20
...
The nesting does not depend on `mv`, so each of these is proven
...
`mv` is entry-constant, so one three-way `ifBit` chain wraps the
...
code, wrapping the three straight-line arms. -/
...
`S1CardEmit.emitLoop_run` does not fit the entry loop. Its
...
invariant that carries an abstract state.
...
/-- **The `stepBlocks` summand as the entry loop produces it.** -/

...
/-!
# FSAT to SAT: the run lemmas

`buildSAT_run` and the size bounds of the program's input and output.
-/
...
/-! ## Run lemmas: the machine folds compute `mScan (serF f)`

Each gadget's `Cmd` writes exactly its

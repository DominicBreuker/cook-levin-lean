...
/-!
# Tableau reduction: parsing and guard

Stage P parses the flattened machine from the input register into unary registers
(alphabet size, number of states, start state, halt bits, transition entries); stage G
computes the guard `S1Map.s1GuardB` into a flag register.
-/
...
open CookLevin.Lang HeadLayout
...
def PTRANS  : Var := 13
def SCAN    : Var := 14
-- 15 = `NumGadgets.HEAD`, 16 = `NumGadgets.INBLK` (reserved)
def FLG     : Var := 17
def VAL     : Var := 18
...
def RES     : Var := 20
def ONE     : Var := 21
-- 22 = `NumGadgets.LT_B` (reserved)
def TSCAN   : Var := 23
def NEF     : Var := 24
-- 26 = `NumGadgets.SKIPR` (reserved)
def I1      : Var := 27
def I2      : Var := 28
def I3      : Var := 29
def I4      : Var := 30
...
`NumGadgets.readNum` consumes a *bare* terminated unary block `1^v 0`; an
...
abbrev RegOK (dst stream idx : Var) : Prop :=
  ...
  stream ≠ NumGadgets.HEAD ∧ stream ≠ NumGadgets.INBLK ∧ stream ≠ NumGadgets.SKIPR ∧
  dst ≠ NumGadgets.HEAD ∧ dst ≠ NumGadgets.INBLK ∧ dst ≠ NumGadgets.SKIPR ∧
  idx ≠ NumGadgets.HEAD ∧ idx ≠ NumGadgets.INBLK ∧ idx ≠ NumGadgets.SKIPR

/-- Drain one `encSyms` item `1 1^v 0` off `stream` into `dst` (as `1^v`). -/
def readItem (dst stream idx : Var) : Cmd :=
  Cmd.op (.tail stream stream) ;; NumGadgets.readNum dst stream idx
...
theorem readItem_run (st : State) (v : Nat) (rest : List Nat)
  ...
  have e1 : (readItem dst stream idx).eval st
      = (NumGadgets.readNum dst stream idx).eval
...
/-- `FLG := FLG && (|A| < |B|)`. -/
def ltCheck (A B idx : Var) : Cmd := NumGadgets.ltBit RES A B idx ;; andIn
...
theorem ltCheck_run (A B idx : Var) (a b : Nat) (p : Bool) (s : State)
    (hALT : A ≠ NumGadgets.LT_B) (hidx : idx ≠ NumGadgets.LT_B)
    (hiF : idx ≠ FLG)
    (hA : s.get A = List.replicate a 1) (hB : s.get B = List.replicate b 1)
    (hF : s.get FLG = (if p then [1] else [])) :
    ((ltCheck A B idx).eval s).get FLG = (if p && decide (a < b) then [1] else []) := by
  obtain ⟨hlt, hfr⟩ := NumGadgets.ltBit_run s a b RES A B idx hA hB hALT hidx
  have hFmid : ((NumGadgets.ltBit RES A B idx).eval s).get FLG
      = (if p then [1] else []) := by
    rw [hfr FLG (by decide) (Ne.symm hiF) (by decide)]; exact hF
  have hRmid : ((NumGadgets.ltBit RES A B idx).eval s).get RES
...
theorem eqCheck_run (A B : Var) (a b : Nat) (p : Bool) (s : State)
  ...
  have hRmid : ((Cmd.op (.eqBit RES A B)).eval s).get RES
    ...
    · rw [if_neg (fun hh => h (NumGadgets.replicate_one_eq_iff.mp hh)),
...
theorem stagePG_usesBelow : Cmd.UsesBelow stagePG 32 := by
  simp [stagePG, stageP, pSuf1, pSuf2, pSuf3, pSuf4, pSuf5, pSuf6, pSuf7, pSuf8,
    ...
    ltCheck, eqCheck, andIn, nop, NumGadgets.readNum, NumGadgets.cSkip,
    NumGadgets.ltBit, Cmd.UsesBelow, Op.UsesBelow,
    ZERO, MREG, SREG, PSIG, PTAPES, PSTATES, PSTART, PNHALT, PHALT, PNTRANS,
    PTRANS, SCAN, FLG, VAL, RES, ONE, TSCAN, NEF, I1, I2, I3, I4, I5,
    NumGadgets.HEAD, NumGadgets.INBLK, NumGadgets.SKIPR, NumGadgets.LT_B]
...
`1^maxSize` and `1^steps` where the front program put them, and stage M finds `1`–`5` free
to overwrite. -/

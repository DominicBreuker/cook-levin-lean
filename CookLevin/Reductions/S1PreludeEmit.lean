...
/-!
# Tableau reduction: the prelude cards, program

The program `cPrelude` emitting `S1Prelude.preludeSeg`, as three nested kind loops around
a resolution loop, with its run lemma `cPrelude_run`.
-/
...
/-! ## The model, with `add` folded into the base -/
...
own counter, `cinR`/`coutR` = the cut-seen bit in
and out. -/

...
The program computing `flatTCC_to_flatCC` and the program witness
`flatTCC_reductionLang`. The map is computed without the validity guard: an invalid
`FlatTCC` maps to an equally invalid `FlatCC`, so both sides of the reduction are false.
-/
...
`NumGadgets.readNum` pins `HEAD = 15`, `INBLK = 16`, `SKIPR = 26` (`cSkip`). -/
...
def blockMove : Cmd :=
  ...
  NumGadgets.readNum VALX SCAN IDXR ;;
...
def cardStep : Cmd :=
  ...
  Cmd.ifBit FLAG (halfMove ;; halfMove) NumGadgets.cSkip
...
theorem encKey_injective : Function.Injective encKey := by
  ...
  | mk PS PO PW PI PC PF PT =>
      ...
      | mk QS QO QW QI QC QF QT =>
          ...
          exact ⟨NumGadgets.replicate_one_eq_iff.mp h1,
            NumGadgets.replicate_one_eq_iff.mp h2,
            NumGadgets.replicate_one_eq_iff.mp h3,
            encNats_injective h4, encCardsOut_injective h5,
            encFinal_injective h6, NumGadgets.replicate_one_eq_iff.mp h7⟩
...
theorem blockMove_run (s : State) (v : Nat) (X : List Nat) (S T : Nat)
  ...
  obtain ⟨hVAL, hSC2, hF2⟩ := NumGadgets.readNum_run s1 v X VALX SCAN IDXR
    hs1SCAN (by decide) (by decide) (by decide) (by decide) (by decide)
    (by decide) (by decide) (by decide) (by decide) (by decide) (by decide)
    (by decide)
  set s2 := (NumGadgets.readNum VALX SCAN IDXR).eval s1 with hs2
  ...
  · -- cost accounting
    ...
    have hcost : blockMove.cost s
        = 1 + 1 + (1 + (NumGadgets.readNum VALX SCAN IDXR).cost s1
...
theorem cardStep_idle (s : State) (hSC : State.get s SCAN = []) :
    cardStep.eval s = (s.set FLAG [0]).set NumGadgets.SKIPR [1]
...
def CInv (cs : List (TCCCard Nat)) (s0 : State) (i : Nat) (st : State) : Prop :=
  ...
  ∧ (∀ r : Var, r ≠ SCAN → r ≠ OUT → r ≠ VALX → r ≠ FLAG → r ≠ IDXO →
      r ≠ IDXR → r ≠ NumGadgets.HEAD → r ≠ NumGadgets.INBLK →
      r ≠ NumGadgets.SKIPR → State.get st r = State.get s0 r)
...
theorem cardStep_step (cs : List (TCCCard Nat)) (s0 : State) (S : Nat)
  ...
  · -- card iteration
    ...
    -- (hoisted before any run/cost fact enters the context)
...
theorem cardConvert_usesBelow : Cmd.UsesBelow cardConvert 27 := by
  simp [cardConvert, cardStep, halfMove, blockMove, NumGadgets.readNum,
    NumGadgets.cSkip, Cmd.UsesBelow, Op.UsesBelow, CARDS, OFFSET, WIDTH, OUT,
    SCAN, VALX, FLAG, IDXO, IDXR, NumGadgets.HEAD, NumGadgets.INBLK,
    NumGadgets.SKIPR]

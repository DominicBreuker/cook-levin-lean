...
/-!
# Building blocks of the front program

`emitConst` writes a constant list of cells into a register, `emitRegs` re-encodes the input
registers into the machine alphabet, and `unaryMonomial` writes `1^(c·(n+1)^k + d)` from a
register of length `n`; each with its run and cost lemma.
-/
...
/-- **The constant emitter**: `dst := bits.map bitVal` — for a
...
/-- **The per-symbol re-encoder**: append
...
The machine input string is `s_x = 3 :: encodeRegs (encX x)` (`s_x ++ cert =
encodeTape (encX x ++ certState c)` splits the canonical tape at the certificate
register). The head layout stores it as
...
be a source. `src` registers survive (read-only). -/
...
degree-`k`-monomial shape the `inOPoly` obligations consume. -/
...
— the split-layout input's aggregate size — the monomial argument `n` the front witness
...
corrupt a later register's length read; `dst` is the accumulator). -/
...
/-- **The input-cell counter**: `dst := 1^(Σ_{src ∈ srcs}
...
polynomial in the input cell counts. -/
...
/-! ## Cost bound for `emitRegs`

The input-string emitter's cost bound. Mirrors the `tallyCells_cost` `foldl` induction (per-source cost that
...
contribution. A polynomial in the input register lengths. -/

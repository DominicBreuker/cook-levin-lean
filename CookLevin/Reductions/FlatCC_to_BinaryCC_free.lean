...
The program computing `FlatCC_to_BinaryCC_instance` (guarded by the validity test
`isValidFlattening`, which cannot be dropped because the binary image forgets the alphabet
bound) and the program witness `flatCCBin_reductionLang`.
-/
...
Input layout: `flatTCC_reductionLang`'s exit frame (regs 1–8, reg 3
empty); outputs at 17–21; scratch at 9–14, 23–25. `NumGadgets.readNum` pins
...
def remCheck : Cmd :=
  ...
  Cmd.ifBit TFLG NumGadgets.cSkip setInvalid ;;
  Cmd.op (.tail REM REM)
...
def initStep : Cmd :=
  ...
  Cmd.ifBit TFLG
    (NumGadgets.readNum VALX SCAN IDXR ;; expandBare)
    NumGadgets.cSkip
...
def sentStep : Cmd :=
  ...
  Cmd.ifBit TFLG
    (Cmd.op (.head TFLG SCAN) ;;
     ...
     Cmd.ifBit TFLG
       (NumGadgets.readNum VALX SCAN IDXR ;; expandSent)
       (Cmd.op (.appendZero BOUT)))
    NumGadgets.cSkip
...
def binConvert : Cmd :=
  ...
  Cmd.ifBit FLAG NumGadgets.cSkip
...
theorem remCheck_run (s : State) (v k : Nat) (b : Bool)
    ...
    ∧ (∀ r : Var, r ≠ REM → r ≠ TFLG → r ≠ FLAG → r ≠ IDX2 →
        r ≠ NumGadgets.SKIPR → State.get (remCheck.eval s) r = State.get s r)
  ...
  · -- v < k : the check passes; the branch is `cSkip`
    ...
    have e4 : (Cmd.ifBit TFLG NumGadgets.cSkip setInvalid).eval s3
        = s3.set NumGadgets.SKIPR [1] := by
      rw [Cmd.eval_ifBit_true _ _ _ _ hs3TFLG, NumGadgets.cSkip_eval]
    set s4 := s3.set NumGadgets.SKIPR [1] with hs4
...
theorem expandBare_run (s : State) (v k : Nat) (b : Bool)
    ...
    ∧ (∀ r : Var, r ≠ BINIT → r ≠ REM → r ≠ TFLG → r ≠ FLAG → r ≠ IDX2 →
        r ≠ NumGadgets.SKIPR → State.get (expandBare.eval s) r = State.get s r)
...
theorem expandSent_run (s : State) (v k : Nat) (b : Bool)
    ...
    ∧ (∀ r : Var, r ≠ BOUT → r ≠ REM → r ≠ TFLG → r ≠ FLAG → r ≠ IDX2 →
        r ≠ NumGadgets.SKIPR → State.get (expandSent.eval s) r = State.get s r)
...
theorem initStep_block (s : State) (v : Nat) (X : List Nat) (k : Nat)
    ...
    ∧ (∀ r : Var, r ≠ SCAN → r ≠ BINIT → r ≠ VALX → r ≠ REM → r ≠ TFLG →
        r ≠ FLAG → r ≠ IDX2 → r ≠ IDXR → r ≠ NumGadgets.HEAD →
        r ≠ NumGadgets.INBLK → r ≠ NumGadgets.SKIPR →
  ...
  -- drain the block
  obtain ⟨hVAL, hSC2, hF2⟩ := NumGadgets.readNum_run w v X VALX SCAN IDXR
    hwSC (by decide) (by decide) (by decide) (by decide) (by decide)
    (by decide) (by decide) (by decide) (by decide) (by decide) (by decide)
    (by decide)
  set u1 := (NumGadgets.readNum VALX SCAN IDXR).eval w with hu1
  ...
  · have hrn := NumGadgets.readNum_cost w VALX SCAN IDXR (by decide)
      (by decide) (by decide) (by decide) (by decide)
    have hwSlen : (State.get w SCAN).length ≤ S := by
      rw [hw, State.get_set_ne _ _ _ _ (by decide)]
      exact hS
    have hrn' : (NumGadgets.readNum VALX SCAN IDXR).cost w
        ≤ 2 * (S * S) + 7 * S + 7 := by
    ...
    have hcost : initStep.cost s
        = 1 + 1 + (1 + (1 + (NumGadgets.readNum VALX SCAN IDXR).cost w
...
theorem initStep_idle (s : State) (hSC : State.get s SCAN = []) :
    initStep.eval s = (s.set TFLG [0]).set NumGadgets.SKIPR [1]
...
def IInv (xs : List Nat) (k : Nat) (b0 : Bool) (u : State) (i : Nat)
  ...
  ∧ (∀ r : Var, r ≠ SCAN → r ≠ BINIT → r ≠ VALX → r ≠ REM → r ≠ TFLG →
      r ≠ FLAG → r ≠ IDX2 → r ≠ IDXR → r ≠ IDXO → r ≠ NumGadgets.HEAD →
      r ≠ NumGadgets.INBLK → r ≠ NumGadgets.SKIPR →
...
theorem sentStep_elem (s : State) (v : Nat) (X : List Nat) (k : Nat)
  ...
  -- drain the block
  obtain ⟨hVAL, hSC3, hF3⟩ := NumGadgets.readNum_run w2 v X VALX SCAN IDXR
    hw2SC (by decide) (by decide) (by decide) (by decide) (by decide)
    (by decide) (by decide) (by decide) (by decide) (by decide) (by decide)
    (by decide)
  set u1 := (NumGadgets.readNum VALX SCAN IDXR).eval w2 with hu1
  have hu1get : ∀ r : Var, r ≠ SCAN → r ≠ VALX → r ≠ TFLG →
      r ≠ NumGadgets.INBLK → r ≠ NumGadgets.HEAD → r ≠ NumGadgets.SKIPR →
  ...
  · -- cost
    have hrn := NumGadgets.readNum_cost w2 VALX SCAN IDXR (by decide)
      (by decide) (by decide) (by decide) (by decide)
    have hw2Slen : (State.get w2 SCAN).length ≤ S := by
      rw [hw2SC]
      simp only [List.length_append, List.length_replicate, List.length_cons]
      omega
    have hrn' : (NumGadgets.readNum VALX SCAN IDXR).cost w2
        ≤ 2 * (S * S) + 7 * S + 7 := by
    ...
    have hcost : sentStep.cost s
        = 1 + 1 + (1 + (1 + 1 + (1 + ((State.get w1 SCAN).length + 1)
            + (1 + (1 + (NumGadgets.readNum VALX SCAN IDXR).cost w2
...
theorem sentStep_idle (s : State) (hSC : State.get s SCAN = []) :
    sentStep.eval s = (s.set TFLG [0]).set NumGadgets.SKIPR [1]
...
theorem binConvert_run (k o w : Nat) (xs : List Nat) (cs : List (CCCard Nat))
  ...
  have hu13get : ∀ r : Var, r ≠ SCAN → r ≠ BOUT → r ≠ BCARDS → r ≠ VALX →
      ...
      r ≠ NumGadgets.HEAD → r ≠ NumGadgets.INBLK → r ≠ NumGadgets.SKIPR →
  ...
  have hu17get : ∀ r : Var, r ≠ SCAN → r ≠ BOUT → r ≠ BCARDS → r ≠ BFINAL →
      ...
      r ≠ IDXO → r ≠ NumGadgets.HEAD → r ≠ NumGadgets.INBLK →
      r ≠ NumGadgets.SKIPR → State.get u17 r = State.get u9 r := by
  ...
  have hlenBF : (expandItems k (fitemsOf fss)).length ≤ 3 * (k * LF + LF) := by
    ...
    omega
  set A6 := (Cmd.ifBit FLAG NumGadgets.cSkip
      (Cmd.op (.clear BOFF) ;; Cmd.op (.clear BWID) ;; Cmd.op (.clear BINIT) ;;
  ...
  have hGuard : A6 ≤ 12 := by
    ...
    · rw [Cmd.cost_ifBit_true _ _ _ _ hf, NumGadgets.cSkip_cost]
      omega
  ...
  · -- invalid: the else branch clears the six output registers
    ...
    have eG : (Cmd.ifBit FLAG NumGadgets.cSkip
        (Cmd.op (.clear BOFF) ;; Cmd.op (.clear BWID) ;; Cmd.op (.clear BINIT) ;;
         Cmd.op (.clear BCARDS) ;; Cmd.op (.clear BFINAL) ;;
  ...
  · -- valid: the guard is a `cSkip`
    ...
    have eG : (Cmd.ifBit FLAG NumGadgets.cSkip
        (Cmd.op (.clear BOFF) ;; Cmd.op (.clear BWID) ;; Cmd.op (.clear BINIT) ;;
         Cmd.op (.clear BCARDS) ;; Cmd.op (.clear BFINAL) ;;
         Cmd.op (.clear STEPS))).eval u17 = u17.set NumGadgets.SKIPR [1] := by
      rw [Cmd.eval_ifBit_true _ _ _ _ hFL, NumGadgets.cSkip_eval]
    have heval : binConvert.eval s0 = u17.set NumGadgets.SKIPR [1] := by
    ...
    · intro r hr _
      have hge : ∀ m : Nat, 9 ≤ m → r ≠ m :=
        fun m hm => Nat.ne_of_lt (Nat.lt_of_lt_of_le hr hm)
      rw [heval,
        State.get_set_ne _ NumGadgets.SKIPR _ _ (hge 26 (by omega)),
        hu17low r hr]
...
theorem binConvert_usesBelow : Cmd.UsesBelow binConvert 27 := by
  simp [binConvert, initStep, sentStep, expandBare, expandSent, remCheck,
    setInvalid, NumGadgets.readNum, NumGadgets.cSkip, Cmd.UsesBelow,
    Op.UsesBelow, SIGMA, INIT, FINAL, STEPS, OFFSET, WIDTH, CARDS, SCAN,
    VALX, FLAG, IDXO, IDXR, REM, BOFF, BWID, BINIT, BCARDS, BFINAL, TFLG,
    IDX2, BOUT, NumGadgets.HEAD, NumGadgets.INBLK, NumGadgets.SKIPR]
...
required here: the unguarded map is not correct). -/

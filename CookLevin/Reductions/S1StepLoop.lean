...
/-!
# Tableau reduction: the transition loop

`entryPre` pops one transition entry from the parsed transition stream, decides whether it
is the first entry with its key, and loads its numbers; `stepFam` loops it over all entries
with `stepEmit`, producing the step card family (`stepFam_run`).
-/
...
def LD : List Var :=
  [SCUR, SSEEN, SCNT, SKP, SKQ, SKT, SKV, SAX, SIX,
   ...
   NumGadgets.HEAD, NumGadgets.INBLK, NumGadgets.SKIPR]
...
/-- The seen stream is at least as long as the seen list. -/
...
A cursor loop over the seen register. `scanBody` is guarded on its own cursor, so the bound
only has to be an **upper** bound on `|seen|` — and `SSEEN` itself is one
, which is what makes the scan fit the exhausted register licence. -/
...
/-- **The entry loop's body**, guarded on its own cursor. -/
def entryBody : Cmd :=
...
theorem entryPre_usesBelow : Cmd.UsesBelow entryPre 48 := by
  simp [entryPre, preSrc, preKey, preDst, preMv, hvBlk, haltBlk, dropLoop, optRead,
    ...
    S1Prelude.minBody, setTrue, S1Parse.readItem, NumGadgets.readNum, NumGadgets.cSkip,

...
/-!
# FlatCC to BinaryCC: the map

Symbols of `Fin k` are encoded in unary as bit blocks of length `k`
(`encodeSymbol`, `encodeString`), cards and final strings blockwise, and the covering
problem over `Fin k` is turned into the binary one (`CC_to_BinaryCC`,
`FlatCC_to_BinaryCC_instance` on the flat version). `CC_to_BinaryCC_lang` and
`BinaryCC_to_CC_lang` prove the two directions of correctness, and
`FlatCC_to_BinaryCC_instance_size_bound` bounds the output size.
-/

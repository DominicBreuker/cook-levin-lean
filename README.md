...
| `#print axioms FSATSATFree.fsatToSat_correct` | **`[propext, Classical.choice, Quot.sound]`** — the last tail step `FSAT → SAT` is **mid-flight** (2026-07-15): the machine-friendly map (pre-order positional Tseytin over the Polish `serF` stream, `NP/FSAT_to_SAT_pre.lean`) is **proven correct**, and the **entire run-lemma ladder is now DONE** — the pure scan model = tree map (`mScan_eq_fsatToSat`), all step-1(ii) leaves, and the four loop assemblies `subtreeScan_run` (Dyck invariant) / `tokenBody_run` / `outerLoop_run` / `buildSAT_run`, so `FSATSATFree.buildSAT_run : (buildSAT.eval [serF f]).get CNFOUT = encodeCnf (fsatToSat f)` is proven & axiom-clean (`Reductions/FSAT_to_SAT_free.lean`). Only the cost lemmas, the mechanical `PolyTimeComputableLang` fields, the witness, and the seam remain. |

...
| `#print axioms FSATSATFree.fsatToSat_correct` | **`[propext, Classical.choice, Quot.sound]`** — the last tail step `FSAT → SAT` is **mid-flight** (2026-07-12-b): the machine-friendly map (pre-order positional Tseytin over the Polish `serF` stream, `NP/FSAT_to_SAT_pre.lean`) is **proven correct**, and the full reduction program `buildSAT` is written & `#eval`-validated (`Reductions/FSAT_to_SAT_free.lean`, `probes/FSATPreProbe.lean`). Run/cost lemmas, the witness, and the seam remain. |

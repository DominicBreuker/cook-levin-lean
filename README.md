...
| Genuine `sorry`s in built code | **6** (unchanged; Group C — completion; every `Reductions/S1*.lean` file except `S1Witness.lean` is now sorry-free). Five pre-existing: `red_inNP`'s `inTimePoly` half, `hasDeciderClassical`, 3× MultiToSingle (dead code). One cost obligation: `S1Witness.s1Program_polyCost` (the whole-program cost ladder, restated 2026-07-28-c as the *structural* `Cmd.PolyCost s1Program` — no register table, no invariants, no `_run` lemma) — **the only `sorry` between the current state and an axiom-clean `NPhard'' SAT`**. **`Simulators/CookTableau.lean` and `Simulators/GuessTableau.lean` are fully `sorry`-free** — the S1 bijection `cookTableau_correct`, the cert-guess `guessTableau_correct`, and **both size bounds** are sorry-free & axiom-clean (2026-07-18…-24). |
...
- **S1 (front reduction).** `FlatSingleTMGenNP ⪯p FlatTCC`
  ...
  **The two remaining seams landed 2026-07-27** (`Reductions/S1_to_FlatTCC_comp.lean`,
  ...
  assembly and `S1Witness.s1Program_polyCost` — nothing structural.
  ...
  **`stepBlocks`'s entry body landed 2026-07-28** (`Reductions/S1StepEmit.lean`
  ...
  ladder `S1Witness.s1Program_polyCost` is the only S1 obligation left**, and it

...
| `#print axioms FSATSATFree.fsatToSat_correct` | **`[propext, Classical.choice, Quot.sound]`** — the last tail step `FSAT → SAT` is **mid-flight** (2026-07-12-c): the machine-friendly map (pre-order positional Tseytin over the Polish `serF` stream, `NP/FSAT_to_SAT_pre.lean`) is **proven correct**, the full reduction program `buildSAT` is written & `#eval`-validated, and **run-lemma step 1(i) is done** — the pure scan model is PROVEN equal to the tree map (`mScan_eq_fsatToSat`, `Reductions/FSAT_to_SAT_free.lean`), factoring the tree recursion off the pending machine-fold proof. Machine folds (1(ii)), cost lemmas, the witness, and the seam remain. |

...
| `#print axioms FSATSATFree.fsatToSat_correct` | **`[propext, Classical.choice, Quot.sound]`** — the last tail step `FSAT → SAT` is **mid-flight** (2026-07-13): the machine-friendly map (pre-order positional Tseytin over the Polish `serF` stream, `NP/FSAT_to_SAT_pre.lean`) is **proven correct**, the full reduction program `buildSAT` is written & `#eval`-validated, run-lemma step 1(i) is done (pure scan model = tree map, `mScan_eq_fsatToSat`), and **step 1(ii)'s LEAVES are all proven** — the `encodeCnf` append algebra, the 5 emit gadgets, the 2 sentinel-drain inner loops, and all 6 `budgetBody` cases (⇒ pure `budgetStep`), in `Reductions/FSAT_to_SAT_free.lean`. The loop assemblies (`subtreeScan_run` via a Dyck invariant, `tokenBody_run`, the outer loop), cost lemmas, the witness, and the seam remain. |

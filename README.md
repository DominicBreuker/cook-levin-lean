...
| `#print axioms FrontS1Comp.SAT_NPhard''` / `S1SATComp.s1_to_SAT_reducesPolyMO'` | `[propext, sorryAx, Classical.choice, Quot.sound]` — `NPhard'' SAT` and `FlatSingleTMGenNP ⪯p' SAT` at the *real* S1 program; the `sorryAx` is now **exactly one lemma**, `S1Witness.s1Program_cost_le` (the cost ladder). Measured head-room: `3.6e5` against a budget of `1.1e15`, floor `> 6·10^8` (`probes/S1StepLoopProbe.lean` §4/§5). |
| Genuine `sorry`s in built code | **6** (was 9; Group C — completion; every `Reductions/S1*.lean` file except `S1Witness.lean` is now sorry-free). Five pre-existing: `red_inNP`'s `inTimePoly` half, `hasDeciderClassical`, 3× MultiToSingle (dead code). One cost obligation: `S1Witness.s1Program_cost_le` (the whole-program cost ladder) — **the only `sorry` between the current state and an axiom-clean `NPhard'' SAT`**. **`Simulators/CookTableau.lean` and `Simulators/GuessTableau.lean` are fully `sorry`-free** — the S1 bijection `cookTableau_correct`, the cert-guess `guessTableau_correct`, and **both size bounds** are sorry-free & axiom-clean (2026-07-18…-24). |
...
- **S1 (front reduction).** `FlatSingleTMGenNP ⪯p FlatTCC`
  ...
  **`stepBlocks`'s entry body landed 2026-07-28** (`Reductions/S1StepEmit.lean`
  ...
  ladder `S1Witness.s1Program_cost_le` is the only S1 obligation left**, and it

...
- **S1 (front reduction).** `FlatSingleTMGenNP ⪯p FlatTCC`
  ...
  **The prelude family — `~96%` of the card register and stage C's cost driver
  ...
  found that **stage C's register licence `CDirty` is exactly full** (30
  registers, all used) and that **the cost budget has ~12 orders of magnitude of
  slack**, which fixes how the last two pieces must be built.

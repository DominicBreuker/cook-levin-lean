...
| `#print axioms FrontS1Comp.SAT_NPhard''` / `S1SATComp.s1_to_SAT_reducesPolyMO'` | `[propext, sorryAx, Classical.choice, Quot.sound]` — `NPhard'' SAT` and `FlatSingleTMGenNP ⪯p' SAT` at the *real* S1 program; the `sorryAx` is now **exactly one lemma**, `S1Witness.s1Program_costLeSize` (the cost ladder), whose residual is **two loops in one gadget** (2026-07-29). Measured head-room: `3.6e5` against a budget of `1.1e15`, floor `> 6·10^8` (`probes/S1StepLoopProbe.lean` §4/§5). |
...
| `#print axioms Complexity.Lang.Cmd.capCost_of_capChk` | **`[propext, Classical.choice, Quot.sound]`** — **a cost predicate that survives a loop** (2026-07-29, `Lang/CostGrow.lean`, sorry-free). ⚠ **FINDING Z**: `Cmd.PolyCost`'s single cap over `costReads` *cannot* survive a `forBnd` — after one iteration the body's outputs are bounded by `poly(M)`, so `m` iterations give a **tower**, which is why `Cmd.polyCost_forBnd` has to forbid the body from writing what it cost-reads (the 4% residual). `Cmd.CapCost c F F'` splits the cap in two — a **frozen** set capped by `MF`, everything capped by `N` — and asserts `cost ≤ K·(MF+1)^D·(N+1)`, `∀ r, \|eval@r\| ≤ N + K·(MF+1)^D`, `∀ r ∈ F', \|eval@r\| ≤ MF + K·(MF+1)^D`. Letting the **cost** be linear in `N` while forbidding the **growth** to depend on it is what makes the loop rule non-compounding (`bnd ∈ F` ⇒ trips `≤ MF` ⇒ `N_m ≤ N + MF·poly(MF)`), and it also makes **FINDING X free**: `copy EOUT_C EOUT_C` costs `\|EOUT_C\|+1 ≤ N+1`, so no pinned `_run` lemma has to be re-opened. Closed under `seq` / `ifBit` / `capCost_op` / **`capCost_forBnd`**, with `Cmd.NoGrow` (an *idempotent* `≤ max \|r\| 1`, so a body may be iterated any number of times — the drained cursor), `Cmd.GrowOk` (per-register additive growth) and **`Cmd.promote`** (a written register keeps its cap when `GrowOk` certifies it against the *strictly* frozen set — round 0 buys the invariant, round 1 spends it). `Cmd.capChk` is the decidable forward pass and `Cmd.costLeSize_of_capChk c F (by decide)` the one-liner. Measured (`probes/S1GrowSafeProbe.lean`): it accepts `stagePG`, `stageSig`, `stageInit`, `stageFin`, `stageMYes`, `stageMNo` and `S1CardEmit.cFive`, and over the **whole** program rejects **exactly two loops**, both in `S1StepLoop.scanSeen` — the dedup scan bounded by the seen-set accumulator `SSEEN`. |
| Genuine `sorry`s in built code | **6** (unchanged; Group C — completion; every `Reductions/S1*.lean` file except `S1Witness.lean` is now sorry-free). Five pre-existing: `red_inNP`'s `inTimePoly` half, `hasDeciderClassical`, 3× MultiToSingle (dead code). One cost obligation: `S1Witness.s1Program_costLeSize` (the whole-program cost ladder — no register table, no invariants, no `_run` lemma; restated 2026-07-29 in the shape `Cmd.CapCost` produces, with the residual measured at **two loops**, both in `S1StepLoop.scanSeen`) — **the only `sorry` between the current state and an axiom-clean `NPhard'' SAT`**. **`Simulators/CookTableau.lean` and `Simulators/GuessTableau.lean` are fully `sorry`-free** — the S1 bijection `cookTableau_correct`, the cert-guess `guessTableau_correct`, and **both size bounds** are sorry-free & axiom-clean (2026-07-18…-24). |
